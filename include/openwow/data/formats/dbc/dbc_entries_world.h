...
struct LiquidTypeEntry {
  ...
  std::uint32_t id;
  // Classic stores LiquidType as: ID, LiquidID, Type, SpellID.
  std::uint32_t liquid_id;
  std::uint32_t type;
  std::string_view name;
  std::uint32_t flags;
...
struct HelmetGeosetVisDataEntry {
  std::uint32_t id;
  // Classic 1.12 has five race-mask columns after the id.
  std::array<std::uint32_t, 5> hide_geoset;
