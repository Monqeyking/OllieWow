...
struct CharStartOutfitEntry {
  // Turtle/Vanilla stores 12 equipment slots in a 41-field, 152-byte row.
  // Keep the existing names for now because the catalog API predates the
  // Classic profile split.
  static constexpr std::uint32_t kRetailFieldCount = 41u;
  static constexpr std::uint32_t kRetailRecordSize = 152u;
  static constexpr std::size_t kItemSlotCount = 12u;
