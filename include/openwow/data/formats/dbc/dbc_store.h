...
public:
  DbcStoreLoadResult LoadFromFile(DbcFile file, const DbcTableSchema<T> &schema,
    ...
    constexpr std::size_t kMaxDenseIndexEntries = 1'000'000u;
    const bool use_sparse_index =
        index_width > kMaxDenseIndexEntries ||
        index_width > std::numeric_limits<std::size_t>::max();
    std::vector<std::size_t> index;
    std::unordered_map<std::uint32_t, std::size_t> sparse_index;
    if (use_sparse_index) {
      // Classic IDs are not guaranteed to be dense. Keep memory proportional to
      // the number of rows instead of reserving max_id - min_id slots.
      sparse_index.reserve(entries.size());
    } else {
      index.assign(static_cast<std::size_t>(index_width), kMissingIndex);
    }
    ...
    for (std::size_t entry_index = 0; entry_index < entries.size(); ++entry_index) {
      ...
      if (use_sparse_index) {
        sparse_index[entry.id] = entry_index;
      } else {
    ...
    index_is_sparse_ = use_sparse_index;
  ...
  [[nodiscard]] const T *LookupEntry(std::uint32_t id) const {
    ...
    std::size_t entry_index = kMissingIndex;
    if (index_is_sparse_) {
      ...
      entry_index = index_[static_cast<std::size_t>(id - min_id_)];
    }
...
private:
  ...
  std::unordered_map<std::uint32_t, std::size_t> sparse_index_;
  bool index_is_sparse_ = false;
