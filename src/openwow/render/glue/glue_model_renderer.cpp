...
bool GlueModelRenderer::RenderSingleModel(openwow::ui::glue::GlueWidgetRuntime &widgets,
  ...
  auto *attached_scene =
      attached_scene_binding != nullptr ? attached_scene_binding->scene : nullptr;
  auto m2_path_opt = NormalizeModelPath(widget.model_file);
  auto *assets = static_cast<ModelAssets *>(nullptr);
  if (m2_path_opt.has_value()) {
    EnsureQueued(*m2_path_opt);
    assets = GetAssetsIfReady(*m2_path_opt);
  }
  if (!m2_path_opt.has_value()) {
    return false;
  }
  ...
  std::optional<openwow::render::ModelRenderCallbackLightingState>
      classic_character_lighting;
  if (attached_scene_binding != nullptr && !attached_scene_is_ghost &&
      (callback_lighting == nullptr ||
       !openwow::render::glue::HasRenderableGlueSceneLighting(*callback_lighting))) {
    // CharacterSelect and CharacterCreate use a live glue scene.  Their
    // authored M2/ModelFFX lights remain authoritative when they contain
    // energy; the Classic CharacterModelBase light is only the same fallback
    // used for the attached body when a scene arrives without usable light.
    classic_character_lighting =
        openwow::render::glue::BuildClassicCharacterModelLighting();
    callback_lighting = &*classic_character_lighting;
  }
  ...
  if (attached_scene != nullptr) {
    ...
    auto render_attached =
      ...
      auto submit_attached_geometry =
        ...
        if (!openwow::render::glue::HasRenderableGlueSceneLighting(
                *attached_scene_lighting)) {
          classic_character_lighting =
              openwow::render::glue::BuildClassicCharacterModelLighting();
          attached_scene_lighting = &*classic_character_lighting;
        }
