...
bool ParseRealmConnectionCharEnum(const std::uint8_t* data,
  ...
  for (std::uint8_t i = 0; i < count; ++i) {
    ...
    // Classic/Turtle sends 20 equipment-cache slots. WotLK adds extra slots;
    // those remain zero-initialized in the shared client model.
    constexpr std::size_t kClassicEquipmentSlots = 20;
    for (std::size_t slot_index = 0;
         slot_index < kClassicEquipmentSlots; ++slot_index) {
      auto& slot = entry.equipment[slot_index];
