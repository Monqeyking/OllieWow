...
ParsedRealmList ParseRealmListBinaryDetailed(const std::vector<std::uint8_t>& data,
  ...
  // The active OllieWoW server uses the classic/grunt realm-list packet:
  // the realm count is one byte. Battle.net/WotLK uses a two-byte count.
  if (variant == RealmListVariant::kGrunt) {
    std::uint8_t grunt_count = 0;
    if (!reader.ReadU8(grunt_count)) {
      return parsed;
    }
    count = grunt_count;
  } else if (!reader.ReadU16(count)) {
    return parsed;
  }
  ...
  for (std::uint16_t i = 0; i < count; ++i) {
    ...
    if (variant == RealmListVariant::kGrunt) {
      std::uint32_t classic_type = 0;
      if (!reader.ReadU32(classic_type) || !reader.ReadU8(flags)
          || !reader.ReadCString(realm.name) || !reader.ReadCString(realm.address)
          || !reader.ReadFloat(raw_population)) {
        parsed.realms.clear();
        return parsed;
      }
      type = static_cast<std::uint8_t>(classic_type & 0xFFu);
      realm.type = static_cast<RealmType>(classic_type);
      realm.locked = false;
      realm.is_offline = ((flags & 0x02u) != 0);
      realm.is_new = ((flags & 0x20u) != 0);
      realm.is_recommended = ((flags & 0x40u) != 0);
      realm.is_full = ((flags & 0x80u) != 0);
      realm.is_pvp_flag = realm.type == RealmType::kPvP ||
                          realm.type == RealmType::kRPPvP;
      realm.has_version_data = false;
    } else {
...
// The local Turtle/MaNGOS auth server sends the Classic proof layout:
// M2 (20 bytes) followed by account flags (4 bytes). The WotLK-era survey
// and login flag fields are not present in this response.
constexpr std::size_t kClassicAuthProofBodySize = 24u;
static_assert(kClassicAuthProofBodySize == 20u + sizeof(std::uint32_t));
...
WireReadResult<AuthProofPacket> ReadAuthProof(openwow::net::TcpClient &client,
  ...
  const auto body = ReadExact(client, kClassicAuthProofBodySize, deadline,
                              should_cancel);
  if (body.empty()) {
    return WireFailure<AuthProofPacket>(AuthStatus::kNetworkError,
