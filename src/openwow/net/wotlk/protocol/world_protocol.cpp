...
WorldAuthResult RealmSession::Authenticate(
  ...
  openwow::diagnostics::Log(
      ...
      "RealmSession: CMSG_AUTH_SESSION metadata build=5875"
...
bool RealmSession::ReceivePacketFromStream(
  ...
  if (!stream->incoming_frame.header_decrypted) {
    // Classic MaNGOS sends a fixed four-byte server header
    // (big-endian uint16 size, little-endian uint16 opcode). The classic
    // AuthCrypt state must consume the complete header in one operation.
    stream->incoming_frame.size_field_len = sizeof(std::uint16_t);
    stream->incoming_frame.header_len = sizeof(std::uint16_t) +
                                        sizeof(std::uint16_t);
    if (!read_until(stream->incoming_frame.header_len)) return false;
    if (stream->crypt.IsInitialized()) {
      stream->crypt.DecryptRecv(stream->receive_buffer.data(),
                                stream->incoming_frame.header_len);
    }
    stream->incoming_frame.first_byte_decrypted = true;

    const std::size_t payload_plus_opcode =
