...
WorldPacket PacketSender::BuildAuthSession(const AuthSessionPayload &payload,
  ...
  // Classic/Turtle world auth layout:
  // build, server id, account, client seed, digest, addon data.
  // The extra routing/proof-of-work fields in AuthSessionPayload belong to
  // the WotLK/retail layout and must not be placed on this wire.
...
WorldPacket PacketSender::BuildMoveTimeSkipped(const game::ObjectGuid &mover,
  ...
  pkt.AppendU64(mover.GetRawValue());
