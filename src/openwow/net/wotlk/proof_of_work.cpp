...
std::optional<RealmAuthChallenge> ParseRealmAuthChallenge(
  ...
  if (payload.size() >= RealmAuthChallenge::kWireSize) {
    ...
    // Classic/Turtle world authentication sends only the 32-bit server seed;
    // there is no WotLK proof-of-work block.
    challenge.proof_of_work_difficulty = 0;
    challenge.auth_seed = ReadU32Le(payload, 0);
  }
  return challenge;
}
