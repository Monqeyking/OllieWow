...
OPENWOW_DBC_SCHEMA(CharSectionsEntry,
  ...
  DBC_U32(race_id, 1)
  DBC_U32(sex_id, 2)
  // Classic 1.12 CharSections layout: field 3 is SectionType, field 4 is
  // Variation, and field 5 is Color. The existing consumers use the legacy
  // member names `base_section`, `type`, and `variation` for these three
  // logical positions, respectively.
  DBC_U32(base_section, 3)
  DBC_U32(type, 4)
  DBC_U32(variation, 5)
  DBC_U32(flags, 9)
  DBC_STRING_ARRAY(texture_name, 6)
)
...
OPENWOW_DBC_SCHEMA(CharacterFacialHairStylesEntry,
    ...
    e.geoset[0] = f.GetUInt32(row, 6);
    e.geoset[1] = f.GetUInt32(row, 7);
    e.geoset[2] = f.GetUInt32(row, 8);
)
...
OPENWOW_DBC_SCHEMA(CreatureDisplayInfoExtraEntry,
  ...
  for (std::size_t dbc_index = 0; dbc_index < 10u; ++dbc_index) {
    e.item_display[dbc_index] =
        f.GetUInt32(row, 8u + static_cast<std::uint32_t>(dbc_index));
  }
  DBC_STRING(bake_name, 18)
)
