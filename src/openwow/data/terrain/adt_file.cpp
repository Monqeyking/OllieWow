...
bool ParseMver(const BinaryReader &r, size_t data_offset, uint32_t data_size, AdtFile &adt,
  ...
  adt.version = *v;
  // Turtle's Classic client data uses the Classic ADT revision 19.  The
  // chunk decoder below is deliberately version-agnostic and validates every
  // chunk boundary, so accepting this revision does not relax any read
  // bounds.  Keep the older revisions for the existing WotLK fixtures.
  if (adt.version != 19 && adt.version != 18 && adt.version != 17) {
