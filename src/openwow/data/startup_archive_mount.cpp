...
void CollectStartupPatchMatches(const std::filesystem::path& native_dir,
  ...
  for (const auto& entry : std::filesystem::directory_iterator(native_dir, ec)) {
    ...
    const bool is_regular_file = std::filesystem::is_regular_file(entry, ec);
    if (ec) {
      continue;
    }
    const bool is_patch_directory = std::filesystem::is_directory(entry, ec);
    if (ec || (!is_regular_file && !is_patch_directory)) {
      continue;
    }
...
void MountStartupPatchPath(openwow::vfs::VirtualFileSystem* vfs,
                           const std::filesystem::path& patch_path,
                           int priority) {
  std::error_code ec;
  const bool is_directory = std::filesystem::is_directory(patch_path, ec) && !ec;
  const auto kind = is_directory ? openwow::vfs::MountKind::kFilesystem
                                 : openwow::vfs::MountKind::kMpqArchive;
  const auto description = is_directory ? "patch directory" : "patch archive";

  Log(LogLevel::kInfo,
      std::string(is_directory ? "[VFS] Registered " : "[MPQ] Registered ") +
          description +
          " (Storm priority " + std::to_string(priority) + "): " +
          patch_path.string());
  vfs->Mount({
      .id = "patch:" + ToLowerAscii(patch_path.filename().string()) + ":" +
...
void MountStartupArchives(openwow::vfs::VirtualFileSystem* vfs,
  ...
  for (const auto& patch_path : patch_chain) {
    ...
    MountStartupPatchPath(vfs, patch_path, patch_priority++);
  }
