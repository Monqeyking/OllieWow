...
WmoLoadResult LoadWmoRoot(const uint8_t* data, size_t size) {
  ...
  while (cursor + sizeof(ChunkHeader) <= size) {
    ...
    if (!CheckedChunkEnd(cursor, hdr.size, size, chunk_end)) {
      // A small number of Classic root WMOs carry an unframed legacy footer
      // after all required root chunks.  The 1.12-era server extractor stops
      // at EOF here; accept only this narrow, post-header tail case and keep
      // rejecting malformed chunks in the actual root stream.
      if (got_mver && got_mohd && hdr.magic == 0u && size - cursor <= 128u) {
        cursor = size;
        break;
      }
      result.error = "WMO root: chunk extends past end of file at offset " +
                     std::to_string(cursor - sizeof(ChunkHeader)) +
                     " (magic=" + std::to_string(hdr.magic) +
                     ", next=" + std::to_string(r.ReadU32(cursor).value_or(0u)) +
                     ", payload=" + std::to_string(hdr.size) +
