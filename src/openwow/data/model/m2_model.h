...
struct M2Header {
  ...
  // Classic v256 stores the view/profile descriptors in the M2 body.  Later
  // versions keep the profiles in external .skin files and leave this empty.
  M2Array views;
  M2Array colors;
  M2Array textures;
...
// v256 inserts 20 bytes into the fixed header before the bounding-box data.
// Keep enough bytes for both the Classic and WotLK layouts.
inline constexpr std::size_t kM2HeaderBoundsPrefixBytes = 236u;
...
// Decode one Classic v256 embedded view into the normalized SKIN model used
// by the renderer.  The input remains bounds-checked and is never retained.
