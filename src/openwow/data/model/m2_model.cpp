...
constexpr std::uint32_t kClassicM2Version = 256u;
constexpr std::uint32_t kWotlkM2Version = 264u;
...
std::optional<M2TrackHeader> ReadTrackHeader(const BinaryReader &r, std::size_t off,
  ...
  std::optional<M2Array> ranges;
  if (classic) {
    ranges = ReadM2Array(r, &cursor);
  }
  const auto times = ReadM2Array(r, &cursor);
  const auto values = ReadM2Array(r, &cursor);
  if (!times.has_value() || !values.has_value() || (classic && !ranges.has_value()))
    return std::nullopt;
  return M2TrackHeader{
      .interpolation = *interp,
      .global_sequence = *global_seq,
      .ranges = ranges.value_or(M2Array{}),
      .times = *times,
      .values = *values,
      .has_ranges = classic};
}
...
static_assert(sizeof(M2Range) == 8, "M2Range expected to be 8 bytes.");

M2Quat16 EncodeClassicQuaternion(const M2Quat &value) {
  const auto encode = [](const float component) -> std::int16_t {
    const float finite = std::isfinite(component) ? component : 0.0f;
    const float clamped = std::clamp(finite, -1.0f, 1.0f);
    const auto raw = static_cast<std::uint16_t>(std::lround((clamped + 1.0f) * 32767.5f));
    return static_cast<std::int16_t>(raw);
  };
  return M2Quat16{encode(value.x), encode(value.y), encode(value.z), encode(value.w)};
}
...
bool ReadTrackData(const BinaryReader &r, const M2TrackHeader &h,
  ...
  if (h.has_ranges) {
    const std::size_t range_count = static_cast<std::size_t>(h.ranges.count);
    if (range_count > 4096u) {
      if (error) *error = "M2: Classic track range count exceeds limit";
      return false;
    }
    ...
    std::vector<T> values;
    if constexpr (std::is_same_v<T, M2Quat16>) {
      const auto raw_values = r.ReadVector<M2Quat>(static_cast<std::size_t>(h.values.offset),
                                                   static_cast<std::size_t>(h.values.count));
      if (!raw_values.has_value()) {
        if (error) *error = "M2: Classic quaternion keys out of bounds";
        return false;
      }
      values.reserve(raw_values->size());
      for (const M2Quat &value : *raw_values) {
        values.push_back(EncodeClassicQuaternion(value));
      }
    } else {
      if (h.values.count > std::numeric_limits<std::size_t>::max() / values_per_key) {
        if (error) *error = "M2: Classic track value count overflows";
        return false;
      }
      const auto raw_values = r.ReadVector<T>(
          static_cast<std::size_t>(h.values.offset),
          static_cast<std::size_t>(h.values.count) * values_per_key);
      if (!raw_values.has_value()) {
    ...
    // Classic global-sequence tracks use the flat timestamp/value arrays and
    // do not select a per-animation range.  The 1.12 client (and Benilla)
    // samples these keys directly; in particular this is how eyelid blink
    // scales and animated scene-light intensities are represented.  Some
    // valid Classic files carry no ranges at all, so treating an empty range
    // list as an empty track silently loses those animations.
    if (h.global_sequence >= 0) {
      ...
      if (key_count > std::numeric_limits<std::size_t>::max() / values_per_key) {
        if (error) *error = "M2: Classic global track key count overflows";
        return false;
      }
      const std::size_t value_count = key_count * values_per_key;
      if (value_count > values.size()) {
        if (error) *error = "M2: Classic global track values out of bounds";
        return false;
      }
      out->AppendSet(
          std::span<const std::uint32_t>(times->data(), key_count),
    ...
    for (std::size_t i = 0; i < range_count; ++i) {
      ...
      if (value_start > values.size() || value_count > values.size() - value_start) {
        if (error) *error = "M2: Classic track value range out of bounds";
        return false;
      }
...
constexpr std::size_t kClassicM2ParticleEmitterStride = 0x1F8u;
constexpr std::uint32_t kM2ParticleHeadGeometryFlag = 0x00020000u;
constexpr std::uint32_t kM2ParticleTailGeometryFlag = 0x00040000u;

std::uint16_t EncodeClassicParticleLifetimeTime(const float normalized_time) {
  const float finite_time = std::isfinite(normalized_time) ? normalized_time : 0.5f;
  const float clamped_time = std::clamp(finite_time, 0.0f, 1.0f);
  return static_cast<std::uint16_t>(std::lround(clamped_time * 32767.0f));
}

M2Vec3 DecodeClassicParticleColor(const std::uint32_t packed) {
  return M2Vec3{static_cast<float>((packed >> 16u) & 0xFFu),
                static_cast<float>((packed >> 8u) & 0xFFu),
                static_cast<float>(packed & 0xFFu)};
}

std::uint16_t DecodeClassicParticleAlpha(const std::uint32_t packed) {
  const auto alpha = static_cast<float>((packed >> 24u) & 0xFFu) / 255.0f;
  return static_cast<std::uint16_t>(std::lround(alpha * 32767.0f));
}

void ReadClassicParticleOverLife(const BinaryReader &r, const std::size_t emitter_offset,
                                 M2ParticleEmitter *emitter) {
  if (emitter == nullptr) {
    return;
  }

  const float mid = r.ReadF32(emitter_offset + 0x14Cu).value_or(0.5f);
  const std::uint16_t mid_time = EncodeClassicParticleLifetimeTime(mid);
  const std::array<std::uint16_t, 3> color_times{0u, mid_time, 32767u};
  const std::array<std::uint16_t, 4> cell_times{0u, mid_time, mid_time, 32767u};
  emitter->color_track.times.assign(color_times.begin(), color_times.end());
  emitter->alpha_track.times.assign(color_times.begin(), color_times.end());
  emitter->scale_track.times.assign(color_times.begin(), color_times.end());

  for (std::size_t index = 0; index < 3u; ++index) {
    const auto packed = r.ReadU32(emitter_offset + 0x150u + index * 4u).value_or(0xFFFFFFFFu);
    emitter->color_track.values.push_back(DecodeClassicParticleColor(packed));
    emitter->alpha_track.values.push_back(DecodeClassicParticleAlpha(packed));
    const float scale = r.ReadF32(emitter_offset + 0x15Cu + index * 4u).value_or(1.0f);
    emitter->scale_track.values.push_back(M2Vec2{scale, scale});
  }

  emitter->head_cell_track.times.assign(cell_times.begin(), cell_times.end());
  emitter->tail_cell_track.times.assign(cell_times.begin(), cell_times.end());
  emitter->head_cell_track.values = {
      r.ReadU16(emitter_offset + 0x168u).value_or(0u),
      r.ReadU16(emitter_offset + 0x16Au).value_or(0u),
      r.ReadU16(emitter_offset + 0x16Eu).value_or(0u),
      r.ReadU16(emitter_offset + 0x170u).value_or(0u),
  };
  emitter->tail_cell_track.values = {
      r.ReadU16(emitter_offset + 0x174u).value_or(0u),
      r.ReadU16(emitter_offset + 0x176u).value_or(0u),
      r.ReadU16(emitter_offset + 0x178u).value_or(0u),
      r.ReadU16(emitter_offset + 0x17Au).value_or(0u),
  };
}
...
std::optional<M2HeaderFixedPrefixParse> ReadM2HeaderFixedPrefix(
  ...
  if (h.version != kClassicM2Version && h.version != kWotlkM2Version) {
  ...
  off += 4;
  if (h.version == kClassicM2Version) {
    // v256 stores an additional 32-bit offset after the view count.  The
    // count has already been consumed above; consuming a full M2Array here
    // would shift colors/textures by four bytes.
    const auto views_offset = r.ReadU32(off);
    if (!views_offset.has_value()) {
      if (error) *error = "M2: truncated views array";
      return std::nullopt;
    }
    h.views = M2Array{.count = h.num_views, .offset = *views_offset};
    off += 4;
  } else {
    h.views = M2Array{};
  }
  ...
  if (h.version == kClassicM2Version) {
    // The Classic client places one unnamed M2Array between texReplace and
    // renderFlags.  It is not exposed by the normalized model, but it must
    // be consumed here or every following descriptor is shifted by 8 bytes.
    if (!ReadM2Array(r, &off)) {
      if (error) *error = "M2: truncated Classic extra array";
      return std::nullopt;
    }
  }
...
M2LoadResult LoadM2FromBytes(const std::vector<std::uint8_t> &bytes,
  ...
  const bool classic = h.version == kClassicM2Version;
  ...
  if (h.textures.count > 0) {
    ...
    if (!r.CanRead(ofs, bytes_needed)) {
      out.error = "M2: textures out of bounds (count=" + std::to_string(count) +
                  ", offset=" + std::to_string(ofs) +
                  ", bytes=" + std::to_string(bytes_needed) +
                  ", file=" + std::to_string(bytes.size()) +
                  ", version=" + std::to_string(h.version) +
                  ", colors=" + std::to_string(h.colors.count) + "/" +
                  std::to_string(h.colors.offset) +
                  ", transparency=" + std::to_string(h.transparencies.count) +
                  "/" + std::to_string(h.transparencies.offset) + ")";
      return out;
    }
  ...
  if (h.animations.count > 0) {
    ...
    const std::size_t stride = classic ? 68u : M2AnimationSequenceRecord::kStride;
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      const auto length_or_end = r.ReadU32(entry + (classic ? 8u : 4u));
      const auto move_speed = r.ReadF32(entry + (classic ? 12u : 8u));
      const auto flags = r.ReadU32(entry + (classic ? 16u : 12u));
      const auto frequency = r.ReadI16(entry + (classic ? 20u : 16u));

      const auto replay_min = r.ReadU32(entry + (classic ? 24u : 20u));
      const auto replay_max = r.ReadU32(entry + (classic ? 28u : 24u));
      const auto blend_time_ms = r.ReadU32(entry + (classic ? 32u : 28u));
      const auto bounding_box_min_x = r.ReadF32(entry + (classic ? 36u : 32u));
      const auto bounding_box_min_y = r.ReadF32(entry + (classic ? 40u : 36u));
      const auto bounding_box_min_z = r.ReadF32(entry + (classic ? 44u : 40u));
      const auto bounding_box_max_x = r.ReadF32(entry + (classic ? 48u : 44u));
      const auto bounding_box_max_y = r.ReadF32(entry + (classic ? 52u : 48u));
      const auto bounding_box_max_z = r.ReadF32(entry + (classic ? 56u : 52u));
      const auto bounding_radius = r.ReadF32(entry + (classic ? 60u : 56u));
      const auto next_animation = r.ReadI16(entry + (classic ? 64u : 60u));
      const auto alias_next = r.ReadU16(entry + (classic ? 66u : 62u));
      if (!animation_id || !sub_animation_id || !move_speed || !flags || !frequency ||
          !length_or_end || !replay_min || !replay_max || !blend_time_ms || !bounding_box_min_x ||
      ...
      out.model.animation_durations_ms[i] = sequence.length_ms;
    }
  ...
  if (h.bones.count > 0) {
    ...
    const std::size_t stride = classic ? 108u : 88u;
    if (!r.CanRead(base, count * stride)) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      const std::size_t track_base = classic ? 12u : 16u;
      const std::size_t track_stride = classic ? 28u : 20u;
      auto t_header = ReadTrackHeader(r, b + track_base, classic);
      auto r_header = ReadTrackHeader(r, b + track_base + track_stride, classic);
      auto s_header = ReadTrackHeader(r, b + track_base + track_stride * 2u, classic);
      ...
      const std::size_t pivot_offset = classic ? 96u : 76u;
      const auto px = r.ReadF32(b + pivot_offset + 0u);
      const auto py = r.ReadF32(b + pivot_offset + 4u);
      const auto pz = r.ReadF32(b + pivot_offset + 8u);
      if (!px || !py || !pz) {
  ...
  if (h.colors.count > 0) {
    ...
    const std::size_t kTrackHeaderSize = classic ? 28u : 20u;
    const std::size_t stride = kTrackHeaderSize * 2u;
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      const auto color_header = ReadTrackHeader(r, entry, classic);
      const auto alpha_header = ReadTrackHeader(r, entry + kTrackHeaderSize, classic);
  ...
  if (h.transparencies.count > 0) {
    ...
    const std::size_t stride = classic ? 28u : 20u;
    if (!r.CanRead(base, count * stride)) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      const auto header = ReadTrackHeader(r, base + i * stride, classic);
      if (!header.has_value()) {
  ...
  if (h.uv_animations.count > 0) {
    ...
    const std::size_t stride = classic ? 84u : 60u;
    if (!r.CanRead(base, count * stride)) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      const std::size_t track_stride = classic ? 28u : 20u;
      const auto trans_h = ReadTrackHeader(r, t + 0, classic);
      const auto rot_h = ReadTrackHeader(r, t + track_stride, classic);
      const auto scale_h = ReadTrackHeader(r, t + track_stride * 2u, classic);
  ...
  if (h.cameras.count > 0) {
    ...
    const std::size_t stride = classic ? 124u : 100u;
    if (!r.CanRead(base, count * stride)) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      const auto pos_h = ReadTrackHeader(r, c + 16, classic);
      const auto tar_h = ReadTrackHeader(r, c + (classic ? 56u : 48u), classic);
      const auto roll_h = ReadTrackHeader(r, c + (classic ? 96u : 80u), classic);
      ...
      const std::size_t pos_base = classic ? 44u : 36u;
      const std::size_t target_base = classic ? 84u : 68u;
      cam.position_base.x = r.ReadF32(c + pos_base + 0u).value_or(0.0F);
      cam.position_base.y = r.ReadF32(c + pos_base + 4u).value_or(0.0F);
      cam.position_base.z = r.ReadF32(c + pos_base + 8u).value_or(0.0F);
      cam.target_base.x = r.ReadF32(c + target_base + 0u).value_or(0.0F);
      cam.target_base.y = r.ReadF32(c + target_base + 4u).value_or(0.0F);
      cam.target_base.z = r.ReadF32(c + target_base + 8u).value_or(0.0F);
  ...
  if (h.lights.count > 0) {
    ...
    const std::size_t kTrackHeaderSize = classic ? 28u : 20u;
    const std::size_t kLightStride = 16u + 7u * kTrackHeaderSize;
    if (r.CanRead(base, count * kLightStride)) {
      ...
      for (std::size_t i = 0; i < count; ++i) {
        ...
        const std::size_t track_stride = classic ? 28u : 20u;
        const auto amb_color_h = ReadTrackHeader(r, lo + 16, classic);
        const auto amb_intens_h = ReadTrackHeader(r, lo + 16u + track_stride, classic);
        const auto dif_color_h = ReadTrackHeader(r, lo + 16u + track_stride * 2u, classic);
        const auto dif_intens_h = ReadTrackHeader(r, lo + 16u + track_stride * 3u, classic);
        const auto att_start_h = ReadTrackHeader(r, lo + 16u + track_stride * 4u, classic);
        const auto att_end_h = ReadTrackHeader(r, lo + 16u + track_stride * 5u, classic);
        const auto visibility_h = ReadTrackHeader(r, lo + 16u + track_stride * 6u, classic);
  ...
  if (h.attachments.count > 0) {
    ...
    const std::size_t att_stride = classic ? 48u : 40u;
  ...
  if (h.events.count > 0) {
    ...
    for (std::size_t i = 0; i < count; ++i) {
      ...
      if (classic) {
        // Classic event records use a flat global-timeline timestamp array.
        // Keep the record metadata for the normalized model; event playback
        // is outside the character-entry MVP and must not make model loading
        // depend on optional event data.
      ...
      if (count > 0) {
        ...
        for (std::size_t i = 0; i < count; ++i) {
          ...
          auto read_track = [&](std::size_t off, auto *dst) -> bool {
            const auto th = ReadTrackHeader(r, off, classic);
            if (!th) {
  ...
  if (h.particle_emitters.count > 0) {
    const std::size_t kParticleStride =
        classic ? kClassicM2ParticleEmitterStride : 476u;
    ...
    if (pe_count <= std::numeric_limits<std::size_t>::max() / kParticleStride &&
        r.CanRead(pe_base, pe_count * kParticleStride)) {
      ...
      if (classic) {
        for (std::size_t i = 0; i < pe_count; ++i) {
          ...
          em.id = static_cast<std::int32_t>(r.ReadU32(eo + 0x00u).value_or(0xFFFFFFFFu));
          em.flags = r.ReadU32(eo + 0x04u).value_or(0u);
          em.position[0] = r.ReadF32(eo + 0x08u).value_or(0.0f);
          em.position[1] = r.ReadF32(eo + 0x0Cu).value_or(0.0f);
          em.position[2] = r.ReadF32(eo + 0x10u).value_or(0.0f);
          em.bone = r.ReadU16(eo + 0x14u).value_or(0u);
          em.texture = r.ReadU16(eo + 0x16u).value_or(0u);

          const auto read_model_name = [&](const std::size_t array_off) -> std::string {
            const auto count = r.ReadU32(eo + array_off).value_or(0u);
            const auto offset = r.ReadU32(eo + array_off + 4u).value_or(0u);
            if (count == 0u || !r.CanRead(static_cast<std::size_t>(offset),
                                          static_cast<std::size_t>(count))) {
              return {};
            }
            auto text = r.ReadString(static_cast<std::size_t>(offset),
                                     static_cast<std::size_t>(count));
            if (!text.has_value()) {
              return {};
            }
          ...
          em.geometry_model_name = read_model_name(0x18u);
          em.recursion_model_name = read_model_name(0x20u);

          em.blending_type = r.ReadU8(eo + 0x28u).value_or(0u);
          em.emitter_type = static_cast<std::uint8_t>(r.ReadU16(eo + 0x2Au).value_or(0u));
          const auto particle_type = r.ReadU16(eo + 0x2Cu).value_or(0u);
          if (particle_type == 0u || particle_type == 2u) {
            em.flags |= kM2ParticleHeadGeometryFlag;
          }
          if (particle_type == 1u || particle_type == 2u) {
            em.flags |= kM2ParticleTailGeometryFlag;
          }
          em.texture_tile_rows = r.ReadU16(eo + 0x30u).value_or(1u);
          em.texture_tile_cols = r.ReadU16(eo + 0x32u).value_or(1u);

          const auto read_classic_track = [&](const std::size_t track_off,
                                              M2Track<float> &track) {
            const auto header = ReadTrackHeader(r, eo + track_off, true);
            if (!header.has_value()) {
              return;
            }
            std::string ignored_error;
            (void)ReadTrackData(r, *header, &external_sequence_sources, &track, &ignored_error);
          };
          read_classic_track(0x34u, em.emission_speed);
          read_classic_track(0x50u, em.speed_variation);
          read_classic_track(0x6Cu, em.vertical_range);
          read_classic_track(0x88u, em.horizontal_range);
          read_classic_track(0xA4u, em.gravity);
          read_classic_track(0xC0u, em.lifespan);
          read_classic_track(0xDCu, em.emission_rate);
          read_classic_track(0xF8u, em.emission_area_length);
          read_classic_track(0x114u, em.emission_area_width);
          read_classic_track(0x130u, em.z_source);
          ReadClassicParticleOverLife(r, eo, &em);

          em.tail_length = r.ReadF32(eo + 0x17Cu).value_or(0.0f);
          em.twinkle_speed = r.ReadF32(eo + 0x180u).value_or(0.0f);
          em.twinkle_percent = r.ReadF32(eo + 0x184u).value_or(0.0f);
          em.twinkle_scale_min = r.ReadF32(eo + 0x188u).value_or(0.0f);
          em.twinkle_scale_max = r.ReadF32(eo + 0x18Cu).value_or(0.0f);
          em.drag = r.ReadF32(eo + 0x194u).value_or(0.0f);
          em.spin = r.ReadF32(eo + 0x198u).value_or(0.0f);
          for (std::size_t component = 0; component < 3u; ++component) {
            em.tumble_min[component] =
                r.ReadF32(eo + 0x19Cu + component * 4u).value_or(0.0f);
            em.tumble_max[component] =
                r.ReadF32(eo + 0x1A8u + component * 4u).value_or(0.0f);
          }
          em.follow_speed1 = r.ReadF32(eo + 0x1C4u).value_or(0.0f);
          em.follow_scale1 = r.ReadF32(eo + 0x1C8u).value_or(0.0f);
          em.follow_speed2 = r.ReadF32(eo + 0x1CCu).value_or(0.0f);
          em.follow_scale2 = r.ReadF32(eo + 0x1D0u).value_or(0.0f);

          const std::uint32_t spline_point_count =
              r.ReadU32(eo + 0x1D4u).value_or(0u);
          const std::uint32_t spline_point_offset =
              r.ReadU32(eo + 0x1D8u).value_or(0u);
          if (spline_point_count > 0u) {
            const auto points = r.ReadVector<M2Vec3>(
                static_cast<std::size_t>(spline_point_offset),
          ...
          const auto enabled_track = ReadTrackHeader(r, eo + 0x1DCu, true);
          if (enabled_track.has_value()) {
            std::string ignored_error;
            (void)ReadTrackData(r, *enabled_track, &external_sequence_sources, &em.enabled_in,
                                &ignored_error);
          }
        ...
        auto read_track = [&](std::size_t track_off, M2Track<float> &track) {
          const auto th = ReadTrackHeader(r, eo + track_off, classic);
          if (th.has_value()) {
        ...
        const auto enabled_track = ReadTrackHeader(r, eo + 0x1C8, classic);
        if (enabled_track.has_value()) {
...
SkinLoadResult LoadClassicEmbeddedSkinFromM2Bytes(const std::vector<std::uint8_t> &bytes,
                                                  const std::uint32_t profile) {
  SkinLoadResult out;
  BinaryReader r(bytes.data(), bytes.size());
  std::string header_error;
  const auto prefix = ReadM2HeaderFixedPrefix(r, &header_error);
  if (!prefix.has_value()) {
    out.error = std::move(header_error);
    return out;
  }
  if (prefix->header.version != kClassicM2Version || prefix->header.views.count == 0u ||
      profile >= prefix->header.views.count) {
    out.error = "Skin: Classic embedded profile unavailable";
    return out;
  }

  constexpr std::size_t kProfileStride = 44u;
  const std::size_t profile_offset = static_cast<std::size_t>(prefix->header.views.offset) +
                                      static_cast<std::size_t>(profile) * kProfileStride;
  if (!r.CanRead(profile_offset, kProfileStride)) {
    out.error = "Skin: Classic embedded profile out of bounds";
    return out;
  }

  const auto read_profile_array = [&](const std::size_t field) -> M2Array {
    return M2Array{.count = r.ReadU32(profile_offset + field).value_or(0u),
                   .offset = r.ReadU32(profile_offset + field + 4u).value_or(0u)};
  };
  const M2Array indices = read_profile_array(0u);
  const M2Array triangles = read_profile_array(8u);
  const M2Array properties = read_profile_array(16u);
  const M2Array submeshes = read_profile_array(24u);
  const M2Array batches = read_profile_array(32u);
  const std::uint32_t bone_count = r.ReadU32(profile_offset + 40u).value_or(0u);

  SkinHeader skin_header{};
  std::memcpy(skin_header.magic, "SKIN", 4u);
  skin_header.n_indices = indices.count;
  skin_header.ofs_indices = indices.offset;
  skin_header.n_triangles = triangles.count;
  skin_header.ofs_triangles = triangles.offset;
  skin_header.n_properties = properties.count;
  skin_header.ofs_properties = properties.offset;
  skin_header.n_submeshes = submeshes.count;
  skin_header.ofs_submeshes = submeshes.offset;
  skin_header.n_texture_units = batches.count;
  skin_header.ofs_texture_units = batches.offset;
  skin_header.lod = profile;
  out.skin.header = skin_header;

  if (!ReadOptionalSkinChunk<std::uint16_t>(r, indices.count, indices.offset,
                                            &out.skin.indices) ||
      !ReadOptionalSkinChunk<std::uint16_t>(r, triangles.count, triangles.offset,
                                            &out.skin.triangles) ||
      !ReadOptionalSkinPropertyBytes(r, properties.count, properties.offset,
                                     &out.skin.properties)) {
    out.error = "Skin: Classic embedded geometry out of bounds";
    return out;
  }

  constexpr std::size_t kSubmeshStride = 32u;
  if (!r.CanRead(static_cast<std::size_t>(submeshes.offset),
                 static_cast<std::size_t>(submeshes.count) * kSubmeshStride)) {
    out.error = "Skin: Classic embedded submeshes out of bounds";
    return out;
  }
  out.skin.submeshes.reserve(submeshes.count);
  for (std::size_t i = 0; i < submeshes.count; ++i) {
    const std::size_t o = static_cast<std::size_t>(submeshes.offset) + i * kSubmeshStride;
    SkinSubmesh submesh;
    submesh.section_id = r.ReadU16(o + 0u).value_or(0u);
    submesh.level = r.ReadU16(o + 2u).value_or(0u);
    submesh.vertex_start = r.ReadU16(o + 4u).value_or(0u);
    submesh.vertex_count = r.ReadU16(o + 6u).value_or(0u);
    submesh.index_start = r.ReadU16(o + 8u).value_or(0u);
    submesh.index_count = r.ReadU16(o + 10u).value_or(0u);
    submesh.bone_count = r.ReadU16(o + 12u).value_or(0u);
    submesh.bone_combo_index = r.ReadU16(o + 14u).value_or(0u);
    submesh.bone_influences = r.ReadU16(o + 16u).value_or(0u);
    submesh.center_bone_index = r.ReadU16(o + 18u).value_or(0u);
    for (int component = 0; component < 3; ++component) {
      submesh.center_pos[component] =
          r.ReadF32(o + 20u + static_cast<std::size_t>(component) * 4u).value_or(0.0f);
    }
    out.skin.submeshes.push_back(submesh);
  }

  constexpr std::size_t kBatchStride = 24u;
  if (!r.CanRead(static_cast<std::size_t>(batches.offset),
                 static_cast<std::size_t>(batches.count) * kBatchStride)) {
    out.error = "Skin: Classic embedded batches out of bounds";
    return out;
  }
  out.skin.texture_units.reserve(batches.count);
  for (std::size_t i = 0; i < batches.count; ++i) {
    const std::size_t o = static_cast<std::size_t>(batches.offset) + i * kBatchStride;
    SkinTextureUnit unit;
    // Vanilla 1.12 stores M2TextureUnit in the embedded Classic view with a
    // different layout than the later external .skin format.  In particular,
    // textureCount is at +0x0e and textureComboIndex is at +0x10; treating
    // these bytes as the WotLK fields makes the render-flags and texture
    // lookups appear out of range.
    unit.flags = r.ReadU8(o + 0u).value_or(0u);
    unit.priority_plane = 0;
    unit.shading = r.ReadU16(o + 2u).value_or(0u);
    unit.submesh_index = r.ReadU16(o + 4u).value_or(0u);
    unit.submesh_index2 = unit.submesh_index;
