...
constexpr std::array kPushPlayerEventRegistrations{
    PushPlayerEventRegistration{TypeID::kPlayer, 0xF6C, 4,
                                ui::game::events::UPDATE_EXHAUSTION},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x840, 4,
                                ui::game::events::PLAYER_XP_UPDATE},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x844, 4,
                                ui::game::events::PLAYER_XP_UPDATE},
    PushPlayerEventRegistration{TypeID::kPlayer, 0xFD4, 0x1C,
                                ui::game::events::PLAYER_DAMAGE_DONE_MODS},
    PushPlayerEventRegistration{TypeID::kPlayer, 0xFF0, 0x1C,
                                ui::game::events::PLAYER_DAMAGE_DONE_MODS},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x100C, 0x1C,
                                ui::game::events::PLAYER_DAMAGE_DONE_MODS},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x1098, 4,
                                ui::game::events::PLAYER_PVP_KILLS_CHANGED},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x10A4, 4,
                                ui::game::events::PLAYER_PVP_KILLS_CHANGED},
    PushPlayerEventRegistration{TypeID::kPlayer, 0x10C4, 4,
...
constexpr std::uint16_t kUnitFieldBytes2ShapeshiftFormOffsetBytes = 0x279;
...
std::vector<std::uint8_t> CollectChangedPlayerInventorySlots(
  ...
  for (const auto field_index : updates.updated_fields) {
    if (field_index < PLAYER_FIELD_INV_SLOT_HEAD ||
        field_index >= PLAYER_FIELD_KEYRING_SLOT_END) {
      continue;
    }
...
bool TryGetVisibleItemEntrySlot(const std::uint16_t field_index, std::uint8_t &slot_index) {
  if (field_index < PLAYER_VISIBLE_ITEM_1_0 ||
      field_index >= PLAYER_VISIBLE_ITEM_1_0 + 19u * 12u) {
    return false;
  }

  const auto relative_index =
      static_cast<std::uint16_t>(field_index - PLAYER_VISIBLE_ITEM_1_0);
  if ((relative_index % 12u) != 0u) {
    return false;
  }

  slot_index = static_cast<std::uint8_t>(relative_index / 12u);
  return true;
}
