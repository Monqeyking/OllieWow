...
CharacterComponentBaseSectionTable BuildCharacterComponentBaseSectionTable(
  ...
  for (const auto &entry : char_sections.entries()) {
    ...
    const auto section_key =
        std::make_tuple(static_cast<int>(entry.race_id), static_cast<int>(entry.sex_id),
                        static_cast<int>(entry.base_section), static_cast<int>(entry.type),
                        static_cast<int>(entry.variation));
    const auto existing_section = table.entries.find(section_key);
    const bool is_standard_section = (entry.flags & 1u) == 0u;
    const bool should_store_section =
        existing_section == table.entries.end() || is_standard_section ||
        (existing_section->second.flags & 1u) != 0u;
    if (should_store_section) {
      table.entries[section_key] = section_entry;
    }

    if (entry.base_section == kCharacterComponentSectionLookupBaseSection &&
        entry.type == kCharacterComponentSectionLookupBaseType) {
      const auto texture_key = std::make_tuple(static_cast<int>(entry.race_id),
                                                static_cast<int>(entry.sex_id),
                                                static_cast<int>(entry.variation));
      const auto existing_texture = table.texture_paths.find(texture_key);
      if (existing_texture == table.texture_paths.end() || is_standard_section ||
          existing_section == table.entries.end() ||
          (existing_section->second.flags & 1u) != 0u) {
        table.texture_paths[texture_key] = section_entry.texture_names[1];
      }
    }
