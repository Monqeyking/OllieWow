...
void CGGameObject_C::FinalizeGameObjectCreateState(
  ...
  if (upd.fields.HasField(GAMEOBJECT_STATE) ||
      upd.fields.HasField(GAMEOBJECT_TYPE_ID) ||
      upd.fields.HasField(GAMEOBJECT_ARTKIT) ||
      upd.fields.HasField(GAMEOBJECT_ANIMPROGRESS)) {
...
std::vector<std::uint16_t> CGGameObject_C::ApplyValuesUpdate(const ValuesUpdate &upd) {
  ...
  const bool state_changed =
      std::find(changed.begin(), changed.end(), GAMEOBJECT_STATE) != changed.end();
  const bool type_changed =
      std::find(changed.begin(), changed.end(), GAMEOBJECT_TYPE_ID) != changed.end();
  const bool art_kit_changed =
      std::find(changed.begin(), changed.end(), GAMEOBJECT_ARTKIT) != changed.end();
  const bool anim_progress_changed =
      std::find(changed.begin(), changed.end(), GAMEOBJECT_ANIMPROGRESS) != changed.end();
  if (state_changed || type_changed || art_kit_changed || anim_progress_changed) {
...
std::uint32_t CGGameObject_C::GetGoBytes1() const {
  return static_cast<std::uint32_t>(GetGoState()) |
         (static_cast<std::uint32_t>(GetGoType()) << 8u) |
         (static_cast<std::uint32_t>(GetGoArtKit()) << 16u) |
         (static_cast<std::uint32_t>(GetGoAnimProgress()) << 24u);
}

GOState CGGameObject_C::GetGoState() const {
  return static_cast<GOState>(GetUInt32(GAMEOBJECT_STATE));
}

GameObjectType CGGameObject_C::GetGoType() const {
  return static_cast<GameObjectType>(GetUInt32(GAMEOBJECT_TYPE_ID));
}

std::uint8_t CGGameObject_C::GetGoArtKit() const {
  return static_cast<std::uint8_t>(GetUInt32(GAMEOBJECT_ARTKIT));
}

std::uint8_t CGGameObject_C::GetGoAnimProgress() const {
  return static_cast<std::uint8_t>(GetUInt32(GAMEOBJECT_ANIMPROGRESS));
}

void CGGameObject_C::SetGoState(GOState state) {
  const auto previous_state_byte = static_cast<std::uint8_t>(GetGoState());
  if (GAMEOBJECT_STATE < fields_.size()) {
    fields_[GAMEOBJECT_STATE] = static_cast<std::uint32_t>(state);
    OnGoStateByteChanged(previous_state_byte);
  }
}
...
void CGGameObject_C::SetGoAnimProgress(std::uint8_t progress) {
  const auto previous_state_byte = static_cast<std::uint8_t>(GetGoState());
  if (GAMEOBJECT_ANIMPROGRESS < fields_.size()) {
    fields_[GAMEOBJECT_ANIMPROGRESS] = progress;
    OnGoStateByteChanged(previous_state_byte);
  }
}
