...
public:
  ...
  [[nodiscard]] float MaxHealthModifier() const { return 0.0f; }
  [[nodiscard]] float HoverHeight() const { return 0.0f; }
  ...
  [[nodiscard]] std::int32_t PositiveStat(std::uint8_t stat) const {
    return stat <= 4u ? Signed(PLAYER_FIELD_POSSTAT0, stat) : 0;
  }
  [[nodiscard]] std::int32_t NegativeStat(std::uint8_t stat) const {
    return stat <= 4u ? Signed(PLAYER_FIELD_NEGSTAT0, stat) : 0;
  }
