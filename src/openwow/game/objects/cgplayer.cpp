...
std::uint32_t CGPlayer_C::GetVisibleItemEntry(std::uint8_t slot) const {
  if (slot >= 19)
    return 0;
  return GetUInt32(static_cast<std::uint16_t>(
      PLAYER_VISIBLE_ITEM_1_0 + slot * 12));
}
...
std::uint16_t CGPlayer_C::GetVisibleItemEnchant(std::uint8_t slot) const {
  if (slot >= 19)
    return 0;
  std::uint32_t val =
      GetUInt32(static_cast<std::uint16_t>(
          PLAYER_VISIBLE_ITEM_1_0 + slot * 12 + 1));
...
std::uint32_t CGPlayer_C::GetVisibleItemAuraVisual(const std::uint8_t slot) const {
  ...
  const auto packed_enchants = GetUInt32(
      static_cast<std::uint16_t>(PLAYER_VISIBLE_ITEM_1_0 + slot * 12 + 1));
...
CGPlayer_C::QuestLogEntry CGPlayer_C::GetQuestLog(std::uint8_t slot) const {
  ...
  const std::uint16_t base = static_cast<std::uint16_t>(
      PLAYER_QUEST_LOG_1_1 + slot * 3);

  entry.quest_id = GetUInt32(base);
  const std::uint32_t counts_and_state = GetUInt32(
      static_cast<std::uint16_t>(base + 1));
  entry.counts[0] = counts_and_state & 0x3Fu;
  entry.counts[1] = (counts_and_state >> 6) & 0x3Fu;
  entry.counts[2] = (counts_and_state >> 12) & 0x3Fu;
  entry.counts[3] = (counts_and_state >> 18) & 0x3Fu;
  entry.state = (counts_and_state >> 24) & 0xFFu;
  entry.timer = GetUInt32(static_cast<std::uint16_t>(base + 2));
