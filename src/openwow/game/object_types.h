...
enum UpdateFlag : std::uint16_t {
  ...
  // Classic 1.12 calls this UPDATEFLAG_HAS_POSITION. Keep both names so
  // existing presentation code can continue to use the stationary alias.
  kUpdateFlagPosition          = 0x0040,
  kUpdateFlagStationaryPosition= 0x0040,
  // Reserved values keep unsupported WotLK-only checks false in Classic.
  kUpdateFlagVehicle           = 0x8000,
  kUpdateFlagRotation          = 0x4000,
};
...
enum MovementFlag : std::uint32_t {
  ...
  kMoveFlagRoot            = 0x00001000,
  kMoveFlagFalling         = 0x00002000,
  kMoveFlagFallingFar      = 0x00004000,
  ...
  // Classic uses 0x00400000 for spline movement and 0x00800000 for can-fly.
  kMoveFlagSplineEnabled   = 0x00400000,
  kMoveFlagDescending      = 0x00800000,
  kMoveFlagCanFly          = 0x00800000,
  kMoveFlagFlying          = 0x01000000,
  kMoveFlagOnTransport     = 0x02000000,
