...
bool SkipUpdateFields(PacketReader &reader) {
  ...
  if (block_count > kMaxValuesMaskBlocks) {
    openwow::diagnostics::Log(openwow::diagnostics::LogLevel::kWarn, "SkipUpdateFields: block_count " +
                                                           ...
                                                           " exceeds max " +
                                                           std::to_string(kMaxValuesMaskBlocks));
    return false;
  }
...
bool ReadMovementInfo(PacketReader &reader, MovementInfo &out) {
  ...
  if (out.HasFlag(kMoveFlagOnTransport)) {
    if (!reader.ReadGuid(out.transport.guid))
      return false;
  ...
  if (out.HasFlag(kMoveFlagSwimming)) {
...
bool ReadLivingMovementBody(PacketReader &reader, MovementUpdate &out) {
  {
    ...
    // Classic 1.12 sends six movement speeds. The client keeps a nine-entry
    // table for later movement code; the unused entries remain zero.
    for (int i = 0; i < 6; ++i) {
    ...
    if (out.movement.HasFlag(kMoveFlagSplineEnabled)) {
      // Classic MoveSpline::PacketBuilder::WriteCreate layout.
      auto &spline = out.spline;
      spline.active = true;
      if (!reader.ReadU32(spline.flags))
        return false;
      if ((spline.flags & 0x00040000u) != 0u) {
        spline.facing_type = SplineFacing::kAngle;
        if (!reader.ReadFloat(spline.facing_angle))
          return false;
      } else if ((spline.flags & 0x00020000u) != 0u) {
        spline.facing_type = SplineFacing::kTarget;
        if (!reader.ReadGuid(spline.facing_target))
          return false;
      } else if ((spline.flags & 0x00010000u) != 0u) {
        spline.facing_type = SplineFacing::kPoint;
        if (!reader.ReadFloat(spline.facing_x) ||
            !reader.ReadFloat(spline.facing_y) ||
            !reader.ReadFloat(spline.facing_z))
          return false;
      }
      if (!reader.ReadU32(spline.time_passed) ||
          !reader.ReadU32(spline.duration) ||
          !reader.ReadU32(spline.spline_id))
        return false;

      std::uint32_t node_count = 0;
      if (!reader.ReadU32(node_count) ||
          reader.Remaining() < 3u * sizeof(float) ||
          node_count >
              (reader.Remaining() - 3u * sizeof(float)) /
                  (3u * sizeof(float)))
        return false;
      spline.waypoints.resize(static_cast<std::size_t>(node_count) * 3u);
      for (float &point : spline.waypoints) {
        if (!reader.ReadFloat(point))
          return false;
      }
      if (!reader.ReadFloat(spline.dest_x) ||
          !reader.ReadFloat(spline.dest_y) ||
          !reader.ReadFloat(spline.dest_z))
        return false;
    }
  }
...
bool ReadMovementUpdate(PacketReader &reader, MovementUpdate &out) {
  ...
  if (!reader.ReadU8(update_flags))
    return false;
  out.update_flags = update_flags;
  ...
  if (out.HasUpdateFlag(kUpdateFlagLiving)) {
    ...
    if (out.HasUpdateFlag(kUpdateFlagPosition)) {
      ...
      if (out.HasUpdateFlag(kUpdateFlagTransport)) {
        out.transport_offset_x = out.position_x;
        out.transport_offset_y = out.position_y;
        out.transport_offset_z = out.position_z;
        out.transport_guid = ObjectGuid{};
      }
...
bool ReadUpdateFields(PacketReader &reader, std::uint16_t field_count, UpdateFieldValues &out) {
  ...
  const std::uint8_t expected_block_count = BitmaskBlockCount(field_count);
  ...
  if (block_count != 0 && (field_count % 32u) != 0u) {
    const std::uint32_t valid_bits = field_count % 32u;
    const std::uint32_t invalid_bits = out.bitmask.back() &
                                       ~((1u << valid_bits) - 1u);
    if (invalid_bits != 0u) {
      return false;
    }
  ...
  for (const std::uint32_t mask : out.bitmask) {
    set_bits += static_cast<std::uint32_t>(std::popcount(mask));
  }
...
bool ParseUpdateObject(const std::uint8_t *data, std::size_t len,
  ...
  std::uint8_t has_transport = 0;
  if (!reader.ReadU8(has_transport)) {
    openwow::diagnostics::Log(
        openwow::diagnostics::LogLevel::kWarn,
        "UpdateObject: failed to read Classic hasTransport byte");
    return false;
  }
  (void)has_transport;
  ...
  for (std::uint32_t i = 0; i < block_count; ++i) {
    ...
    case UpdateType::kValues: {
      ...
      std::optional<std::uint16_t> values_field_count;
  ...
  if (reader.Remaining() != 0u) {
    return LogUpdateObjectParseFailure(reader, block_count, 0xFFu,
                                       "trailing-bytes", len, block_count);
  }
