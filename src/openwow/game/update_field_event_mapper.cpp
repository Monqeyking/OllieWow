...
std::vector<FieldEvent> MapChangedFieldsToEvents(
  ...
  if (is_unit) {
    for (std::uint16_t f : updated_fields) {
      ...
      if (f >= PLAYER_FIELD_POSSTAT0 && f <= PLAYER_FIELD_POSSTAT4) {
        emit(evt::UNIT_STATS, true);
        continue;
      }
      if (f >= PLAYER_FIELD_NEGSTAT0 && f <= PLAYER_FIELD_NEGSTAT4) {
        emit(evt::UNIT_STATS, true);
        continue;
      }
  ...
  if (type_id == TypeID::kPlayer) {
    for (std::uint16_t f : updated_fields) {
      ...
      if (f >= PLAYER_FIELD_INV_SLOT_HEAD &&
          f < PLAYER_FIELD_KEYRING_SLOT_END) {
        continue;
      }

      if (f >= PLAYER_QUEST_LOG_1_1 &&
          f < PLAYER_QUEST_LOG_1_1 + 20 * 3) {
        emit(evt::UNIT_QUEST_LOG_CHANGED, false);
        continue;
      }

      if (f >= PLAYER_VISIBLE_ITEM_1_0 &&
          f < PLAYER_VISIBLE_ITEM_1_0 + 19 * 12) {
      ...
      if (f >= PLAYER_FIELD_COMBAT_RATING_1 &&
          f < PLAYER_FIELD_COMBAT_RATING_1 + 20) {
        emit(evt::UNIT_STATS, true);
        continue;
      }
