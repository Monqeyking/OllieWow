...
std::size_t BeginWidget(ParserContext* ctx, const XmlNode& node) {
  ...
  // Classic templates commonly name a button label $parentText without an
  // explicit parentKey.  Preserve the vanilla button.text convenience alias.
  if (frame.parent_keys.empty() && ToLowerAscii(frame.kind) == "fontstring" &&
      !parent_name.empty() && name == (parent_name + "Text")) {
    frame.parent_keys.push_back("text");
  }
  frame.inherits =
  ...
  if (ToLowerAscii(frame.kind) == "fontstring") {
    const std::string parent_kind_lower = ToLowerAscii(parent_kind);
    const bool is_button_parent =
        parent_kind_lower == "button" || parent_kind_lower == "checkbutton";
    const bool is_named_parent_text =
        !effective_parent_name.empty() &&
        name == (effective_parent_name + "Text");
    if (original_tag_lower == "buttontext" ||
        (is_button_parent && (!has_explicit_name || is_named_parent_text))) {
      frame.region_role = UiFrame::RegionRole::ButtonText;
    } else if (parent_kind_lower == "editbox" && ctx->layer_stack.empty()) {
      frame.region_role = UiFrame::RegionRole::EditBoxText;
    } else if ((parent_kind_lower == "messageframe" ||
                parent_kind_lower == "scrollingmessageframe") &&
