...
inline constexpr std::uint32_t kClassicFrameXmlInterfaceVersion = 11200u;

[[nodiscard]] constexpr bool IsCompatibleFrameXmlInterfaceVersion(
    const std::uint32_t expected, const std::uint32_t actual) noexcept {
  return actual == expected ||
         (expected == kRetailFrameXmlInterfaceVersion &&
          actual == kClassicFrameXmlInterfaceVersion);
}
