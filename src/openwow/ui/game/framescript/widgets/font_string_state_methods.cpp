...
void ApplyFontStringStateMethods(lua_State *L, int table_idx) {
  ...
  // Classic FrameXML uses SetTextFontObject on some FontStrings as an alias
  // for SetFontObject.  Install the same validated implementation so the
  // alias has identical object and font validation semantics.
  lua_getfield(L, fs, "SetFontObject");
  lua_setfield(L, fs, "SetTextFontObject");
