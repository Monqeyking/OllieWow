...
void ApplyButtonMethods(lua_State *L) {
  ...
  // Vanilla/Classic Button exposes this name for the normal label font.
  lua_getfield(L, f, "SetNormalFontObject");
  lua_setfield(L, f, "SetTextFontObject");

  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    const int self = ValidateFrameObjectSelf(Ls, "Button");
    lua_getfield(Ls, self, "__ow_btn_fontstr");
    if (lua_istable(Ls, -1) == 0) {
      lua_pop(Ls, 1);
      return 0;
    }
    const int font_string = lua_absindex(Ls, -1);
    return SharedSetFontWorker(
        Ls, font_string,
        lua_adapter::ScriptObjectDisplayName(Ls, font_string),
        SetFontFailurePolicy::kReturnNil);
  }, 0);
  ...
  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    ...
    const int font_string = lua_absindex(Ls, -1);
    for (int argument = 2; argument <= 5; ++argument) {
      if (!lua_isnumber(Ls, argument)) continue;
      lua_pushnumber(Ls, lua_tonumber(Ls, argument));
      const char *field = argument == 2 ? "__ow_text_r"
                            : argument == 3 ? "__ow_text_g"
                            : argument == 4 ? "__ow_text_b"
                                            : "__ow_text_a";
      lua_setfield(Ls, font_string, field);
    }
    return 0;
  }, 0);
  ...
  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    const int self = ValidateFrameObjectSelf(Ls, "Button");
    for (int argument = 2; argument <= 5; ++argument) {
      if (!lua_isnumber(Ls, argument)) {
        continue;
      }
      lua_pushnumber(Ls, lua_tonumber(Ls, argument));
      const char *field = argument == 2 ? "__ow_btn_highlight_text_r"
                            : argument == 3 ? "__ow_btn_highlight_text_g"
                            : argument == 4 ? "__ow_btn_highlight_text_b"
                                            : "__ow_btn_highlight_text_a";
      lua_setfield(Ls, self, field);
    }
    return 0;
  }, 0);
  lua_setfield(L, f, "SetHighlightTextColor");

  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    const int self = ValidateFrameObjectSelf(Ls, "Button");
    if (lua_isnumber(Ls, 2)) {
      lua_pushnumber(Ls, lua_tonumber(Ls, 2));
      lua_setfield(Ls, self, "__ow_btn_disabled_text_r");
    }
    if (lua_isnumber(Ls, 3)) {
      lua_pushnumber(Ls, lua_tonumber(Ls, 3));
      lua_setfield(Ls, self, "__ow_btn_disabled_text_g");
    }
    if (lua_isnumber(Ls, 4)) {
      lua_pushnumber(Ls, lua_tonumber(Ls, 4));
      lua_setfield(Ls, self, "__ow_btn_disabled_text_b");
    }
    if (lua_isnumber(Ls, 5)) {
      lua_pushnumber(Ls, lua_tonumber(Ls, 5));
      lua_setfield(Ls, self, "__ow_btn_disabled_text_a");
    }
    return 0;
  }, 0);
  lua_setfield(L, f, "SetDisabledTextColor");
  ...
  // Some Classic FrameXML templates use SetHighlight as a lightweight
  // highlight-state setter in addition to SetHighlightTexture.  Keep the
  // state explicit and accept the optional arguments without making this
  // compatibility shim depend on a particular template implementation.
  lua_pushcclosure(L, [](lua_State *Ls) -> int {
    if (!lua_istable(Ls, 1)) {
      return 0;
    }
    lua_pushboolean(Ls, 1);
    lua_setfield(Ls, 1, "__ow_btn_highlight");
    for (int argument = 2; argument <= 5; ++argument) {
      if (!lua_isnumber(Ls, argument)) {
        continue;
      }
      lua_pushnumber(Ls, lua_tonumber(Ls, argument));
      const char *field = argument == 2 ? "__ow_btn_highlight_r"
                            : argument == 3 ? "__ow_btn_highlight_g"
                            : argument == 4 ? "__ow_btn_highlight_b"
                                            : "__ow_btn_highlight_a";
      lua_setfield(Ls, 1, field);
    }
    return 0;
  }, 0);
  lua_setfield(L, f, "SetHighlight");
