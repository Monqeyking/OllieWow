...
void CreateFontStringTable(lua_State *L, int parent_idx) {
  ...
  // Classic XML frequently omits an explicit font on helper labels and
  // supplies it through the client default.  Keep the same safe fallback
  // used by the UI renderer so SetText is valid before template expansion.
  lua_pushstring(L, "Fonts\\FRIZQT__.TTF");
  lua_setfield(L, fs, "__ow_font_path");
  lua_pushnumber(L, 12.0);
  lua_setfield(L, fs, "__ow_font_size");
  lua_pushnumber(L, 12.0);
  lua_setfield(L, fs, "__ow_text_height");
