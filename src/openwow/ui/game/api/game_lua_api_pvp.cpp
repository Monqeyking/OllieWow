...
// 1.12: BATTLEMASTER = 0x800 (Source\src\game\Objects\UnitDefines.h).
constexpr std::uint32_t kCommentatorBattlemasterNpcFlag = 0x00000800u;
...
int LuaGetPVPSessionStats(lua_State *L) {
  ...
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_SESSION_KILLS)));
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_THIS_WEEK_CONTRIBUTION)));
  return 2;
}

int LuaGetPVPThisWeekStats(lua_State *L) {
  const auto *player = GetLocalPlayer(L);
  if (player == nullptr) {
    lua_pushnumber(L, 0);
    lua_pushnumber(L, 0);
  ...
  // These are the Classic player fields.  The similarly named WotLK-era
  // aliases are intentionally unsupported by the local field definition.
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_THIS_WEEK_KILLS)));
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_THIS_WEEK_CONTRIBUTION)));
  return 2;
}
...
int LuaGetPVPYesterdayStats(lua_State *L) {
  ...
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_YESTERDAY_KILLS)));
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_YESTERDAY_CONTRIBUTION)));
  return 2;
}

int LuaGetPVPLastWeekStats(lua_State *L) {
  const auto *player = GetLocalPlayer(L);
  if (player == nullptr) {
    lua_pushnumber(L, 0);
    lua_pushnumber(L, 0);
  ...
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_LAST_WEEK_KILLS)));
  lua_pushnumber(L, static_cast<lua_Number>(
                        player->GetUInt32(PLAYER_FIELD_LAST_WEEK_CONTRIBUTION)));
  return 2;
}
