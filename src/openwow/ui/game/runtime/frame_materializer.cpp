...
int FrameMaterializer::CreateTrackedFrameBinding(
  ...
  // Classic FrameXML exposes child regions through parentKey (for example
  // button.text).  Publish those aliases before OnLoad so legacy handlers
  // can use the documented object shape.
  if (parent_index != 0 && !frame.parent_keys.empty()) {
    for (const auto &parent_key : frame.parent_keys) {
      if (parent_key.empty()) continue;
      lua_pushvalue(lua_, frame_index);
      lua_setfield(lua_, parent_index, parent_key.c_str());
    }
  }
