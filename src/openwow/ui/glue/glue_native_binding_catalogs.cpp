...
// Classic GlueXML uses these values when it builds the race atlas key and
// calls SetSelectedSex().  Some local Turtle GlueConstants.lua revisions do
// not publish them, so provide the standard Classic values without
// introducing any race or class mapping in native code.
constexpr openwow::ui::LuaIntegerGlobal kGlueCharacterCreationConstants[] = {
    {"SEX_MALE", 1},
    {"SEX_FEMALE", 2},
};
...
GlueNativeBindingCatalogs() {
  ...
  catalogs.push_back(openwow::ui::lua::NativeConstantCatalog(
      "ui.surfaces.glue.character_creation", BindingScope::kGlue,
      kGlueCharacterCreationConstants));
  return catalogs;
}
