...
struct GlueGameState {
  ...
  int create_class{1};
  int create_sex{0};
  // Populated from the active CharacterCreate.lua/DBC enumeration for
  // diagnostics and offline contract checks. XML/Lua own the visible widgets.
  std::vector<int> available_create_race_ids;
  std::vector<int> available_create_class_ids;
  int create_skin{0};
