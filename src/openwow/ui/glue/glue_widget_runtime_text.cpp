...
std::string GlueWidgetRuntime::EnsureButtonTextRegion(
  ...
  RegisterWidget({
      ...
      .button_font_file = owner.button_font_file,
      .button_font_flags = owner.button_font_flags,
      .button_font_height_px = owner.button_font_height_px,
      .has_button_font = owner.has_button_font,
      .visible = true,
...
void GlueWidgetRuntime::SetButtonFont(const std::string& name,
  ...
  const std::string normalized_file = Trim(font_file);
  const std::string flags_lower = ToLowerAscii(flags);
  std::string normalized_flags;
  if (flags_lower.find("thickoutline") != std::string::npos ||
      flags_lower.find("thick") != std::string::npos) {
    normalized_flags = "THICKOUTLINE";
  } else if (flags_lower.find("outline") != std::string::npos) {
    normalized_flags = "OUTLINE";
  }
  if (flags_lower.find("monochrome") != std::string::npos) {
    if (!normalized_flags.empty()) {
      normalized_flags += '|';
    }
    normalized_flags += "MONOCHROME";
  }
  it->second.button_font_file = normalized_file;
  it->second.button_font_height_px = std::max(0, height_px);
  it->second.button_font_flags = std::move(normalized_flags);
  it->second.has_button_font = !normalized_file.empty() && height_px > 0;

  const auto text_region = TextRegionForWidget(name);
  if (text_region.empty()) {
    return;
  }
  auto region_it = widgets_.find(text_region);
  if (region_it == widgets_.end()) {
    return;
  }
  region_it->second.button_font_file = it->second.button_font_file;
  region_it->second.button_font_height_px = it->second.button_font_height_px;
  region_it->second.button_font_flags = it->second.button_font_flags;
  region_it->second.has_button_font = it->second.has_button_font;
  MarkFontStringMetricsDirty(text_region);
}
