...
std::string LowerAscii(const std::string_view value) {
  std::string result;
...
std::string FindCharacterModelPath(const openwow::vfs::VirtualFileSystem &vfs,
  ...
  const std::string sex = gender == 0u ? "male" : "female";
  const std::string prefix = LowerAscii(client_prefix);
  const std::string filename_suffix = sex + ".m2";
  std::string best;

  for (const auto &file : vfs.EnumerateFiles("Character", true)) {
    std::string path = file.generic_string();
    std::replace(path.begin(), path.end(), '\\', '/');
    const std::string lower_path = LowerAscii(path);
    const std::size_t filename_start = lower_path.find_last_of('/');
    if (filename_start == std::string::npos || filename_start + 1u >= lower_path.size()) {
      continue;
    }
    const std::size_t sex_start = lower_path.find_last_of('/', filename_start - 1u);
    if (sex_start == std::string::npos ||
        lower_path.substr(sex_start + 1u, filename_start - sex_start - 1u) != sex) {
      continue;
    }
    const std::string filename = lower_path.substr(filename_start + 1u);
    if (filename.size() < prefix.size() + filename_suffix.size() ||
        filename.rfind(filename_suffix) != filename.size() - filename_suffix.size() ||
...
void GlueCharSelectScene::BindAppearanceDbcStores(
    ...
    const openwow::data::dbc::DbcStore<openwow::data::dbc::ChrClassesEntry> *chr_classes) {
  ...
  chr_classes_ = chr_classes;
  character_model_path_cache_.clear();
  character_display_preloads_.clear();
...
void GlueCharSelectScene::BindVfs(const openwow::vfs::VirtualFileSystem *vfs) noexcept {
  vfs_ = vfs;
  character_model_path_cache_.clear();
}

std::string GlueCharSelectScene::CharacterModelPathForRace(
    const std::uint8_t race, const std::uint8_t gender) const {
  const std::uint16_t cache_key =
      static_cast<std::uint16_t>((static_cast<std::uint16_t>(race) << 8u) | gender);
  if (const auto cached = character_model_path_cache_.find(cache_key);
      cached != character_model_path_cache_.end()) {
    return cached->second;
  }

  if (chr_races_ != nullptr) {
    if (const auto *entry = chr_races_->LookupEntry(race); entry != nullptr) {
      const std::string_view client_prefix =
          !entry->client_file_string.empty() ? entry->client_file_string
                                             : entry->model_client_prefix;
      if (client_prefix.empty() || vfs_ == nullptr) {
        return {};
      }
      const std::string prefix_path =
          std::string("Character\\") + std::string(client_prefix) +
          (gender == 0u ? "\\Male\\" : "\\Female\\") +
          std::string(client_prefix) +
          (gender == 0u ? "Male.m2" : "Female.m2");
      if (vfs_->Exists(prefix_path)) {
        character_model_path_cache_[cache_key] = prefix_path;
        return prefix_path;
      }

      const std::string enumerated_path =
          FindCharacterModelPath(*vfs_, client_prefix, gender);
      if (!enumerated_path.empty()) {
        character_model_path_cache_[cache_key] = enumerated_path;
  ...
  // Keep the original preset resolver only for offline fixtures that do not
  // provide ChrRaces data. A loaded DBC entry is authoritative and must not
  // be replaced by a race-ID switch.
  const std::string known_path = openwow::render::CharacterModelPath(race, gender);
  if (vfs_ == nullptr || (!known_path.empty() && vfs_->Exists(known_path))) {
    character_model_path_cache_[cache_key] = known_path;
    return known_path;
  }
  return {};
}

std::string GlueCharSelectScene::RaceDisplayName(
    const std::uint8_t race, const std::uint8_t gender) const {
  if (chr_races_ == nullptr) {
    return {};
  }
  if (const auto *entry = chr_races_->LookupEntry(race); entry != nullptr) {
    return std::string(entry->DisplayNameForSex(gender));
  }
  return {};
}

std::string GlueCharSelectScene::ClassDisplayName(
    const std::uint8_t class_id, const std::uint8_t gender) const {
  if (chr_classes_ == nullptr) {
    return {};
  }
  if (const auto *entry = chr_classes_->LookupEntry(class_id); entry != nullptr) {
    return std::string(entry->DisplayNameForSex(gender));
  }
  return {};
}
...
void GlueCharSelectScene::RefreshCharacterDisplayPreloads(const GlueGameState &gs) {
  ...
  for (std::size_t index = 0; index < gs.characters.size(); ++index) {
    ...
    const std::string model_path = NormalizeM2Path(
        CharacterModelPathForRace(appearance.race, appearance.gender));
