...
public:
  ...
  void BindAppearanceDbcStores(
      ...
      const openwow::data::dbc::DbcStore<openwow::data::dbc::ChrClassesEntry>
          *chr_classes = nullptr);

  void BindVfs(const openwow::vfs::VirtualFileSystem *vfs) noexcept;

  [[nodiscard]] std::string CharacterModelPathForRace(
      std::uint8_t race, std::uint8_t gender) const;
  [[nodiscard]] std::string RaceDisplayName(std::uint8_t race,
                                             std::uint8_t gender) const;
  [[nodiscard]] std::string ClassDisplayName(std::uint8_t class_id,
                                              std::uint8_t gender) const;
