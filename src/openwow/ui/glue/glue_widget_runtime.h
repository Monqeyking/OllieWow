...
struct GlueWidgetState {
  ...
  // Button:SetFont() is a local font override on the embedded button fonts;
  // it is distinct from the named *FontObject styles above.
  std::string button_font_file;
  std::string button_font_flags;
  int button_font_height_px{0};
  bool has_button_font{false};
...
class GlueWidgetRuntime {
 public:
  ...
  void SetButtonFont(const std::string& name, const std::string& font_file,
                     int height_px, const std::string& flags);
