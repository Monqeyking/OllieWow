...
std::string_view LookupGlueRaceClientFileString(lua_State *state, int race_id);
std::string_view LookupGlueClassTag(lua_State *state, int class_id);
...
int LuaGetCharacterInfo(lua_State *state) {
  ...
  const std::string_view race_file_string = LookupGlueRaceClientFileString(
      state, static_cast<int>(character.race_id));
  ...
  // The active local CharacterSelect.lua consumes this position as the DBC
  // ChrRaces client_file_string and passes it to SetBackgroundModel().
  // Gender is the following value (2=male, 3=female), matching that Lua
  // caller's tuple rather than the old WotLK-internal ordering.
  PushLuaStringView(state, race_file_string);
...
// The active Classic CharacterCreate.lua uses 1 for male and 2 for female.
// GlueGameState stores the corresponding zero-based values 0 and 1. The
// previous 2/3/1 mapping belonged to the WotLK-style glue contract and caused
// the post-selection race atlas lookup to use the wrong key.
constexpr int kClassicLuaSexMapping[2] = {1, 2};
...
void CacheAvailableCreateClassIds(
    GlueGameState &game_state,
    const std::vector<GlueCreateClassEntry> &classes) {
  game_state.available_create_class_ids.clear();
  game_state.available_create_class_ids.reserve(classes.size());
  for (const auto &entry : classes) {
    if (entry.has_definition) {
      game_state.available_create_class_ids.push_back(entry.class_id);
    }
  }
}
...
bool LuaClassIconTokenIsAvailable(lua_State *state,
                                  const std::string_view token) {
  if (state == nullptr || token.empty()) {
    return false;
  }

  lua_getglobal(state, "CLASS_ICON_TCOORDS");
  ...
  const bool available = lua_istable(state, -1) != 0;
  lua_pop(state, 2);
  return available;
}
...
std::vector<GlueAvailableRaceInfo> BuildGlueAvailableRacesFromFallback() {
  ...
  for (const auto &race : kWotlkRaces) {
    // This is only the no-DBC/no-Lua-atlas emergency path.  Draenei and
    // Blood Elf are WotLK-only entries in this legacy fallback; the DBC path
    // below admits any race when its active Lua atlas token is present.
    if (race.id == 11 || race.id == 10) {
      continue;
    }
    races.push_back({
        .race_id = race.id,
...
BuildGlueAvailableRacesFromDbc(lua_State *state,
  ...
  const bool has_char_base_info = !dbc.char_base_info().empty();
  ...
  for (const auto &faction_group : faction_groups) {
    ...
    for (const auto &race : dbc.chr_races()) {
      ...
      // Once CharBaseInfo is available, it is the client-side authority for
      // createable race/class pairs.  This keeps unrelated ChrRaces rows out
      // of the finite stock Lua race-slot array while allowing a custom race
      // through the same DBC-only path.
      if (has_char_base_info) {
        const bool has_create_class = std::any_of(
            dbc.char_base_info().entries().begin(),
            dbc.char_base_info().entries().end(),
            [&](const auto &combo) {
              return combo.race_id == race.id &&
                     dbc.chr_classes().LookupEntry(combo.class_id) != nullptr;
            });
        if (!has_create_class) {
          continue;
        }
      ...
      // The active Lua/XML glue owns the icon/token decision.  Do not require
      // RACE_ICON_TCOORDS here: that table may be loaded later, and filtering
      // on it prevents a valid DBC race from reaching the same Lua flow as the
      // stock races.
      const std::string_view race_token =
          LookupGlueRaceClientFileString(state, static_cast<int>(race.id));
      if (race_token.empty()) {
        continue;
      }
...
GlueAvailableRaceList LoadGlueAvailableRaces(lua_State *state) {
  if (const auto *dbc = GetDbcLoader(state); dbc != nullptr) {
    auto races = BuildGlueAvailableRacesFromDbc(state, *dbc);
    if (!races.empty()) {
      return {
...
std::size_t GetGlueIndexedWidgetLimit(lua_State *state, const char *prefix,
                                      std::size_t fallback);

std::vector<GlueAvailableRaceInfo> GetGlueAvailableRaces(lua_State *state) {
  const auto discovered = LoadGlueAvailableRaces(state);
  const auto &discovered_races = discovered.races;
  std::vector<GlueAvailableRaceInfo> races;
  races.reserve(discovered_races.size());
  for (const auto &race : discovered_races) {
    if (discovered.from_dbc) {
      if (!LookupGlueRaceClientFileString(state, race.race_id).empty()) {
        races.push_back(race);
      }
    } else if (!race.tag.empty()) {
      races.push_back(race);
    }
  }

  lua_getglobal(state, "MAX_RACES");
  if (lua_isnumber(state, -1) != 0) {
    const auto max_races = static_cast<std::size_t>(std::max(
        0, static_cast<int>(lua_tonumber(state, -1))));
  ...
  const auto xml_race_limit =
      GetGlueIndexedWidgetLimit(state, "CharacterCreateRaceButton", races.size());
  if (races.size() > xml_race_limit) {
    races.resize(xml_race_limit);
  }

  if (auto *game_state = GetGameState(state); game_state != nullptr) {
    game_state->available_create_race_ids.clear();
    game_state->available_create_race_ids.reserve(races.size());
    for (const auto &race : races) {
      game_state->available_create_race_ids.push_back(race.race_id);
    }
  }
  return races;
}
...
std::vector<int> GetGlueEnumeratedClassIds(lua_State *state) {
  const auto class_ids = GetGlueAvailableClassIds(state);
  std::vector<int> visible_class_ids;
  visible_class_ids.reserve(class_ids.size());
  for (const int class_id : class_ids) {
    if (LuaClassIconTokenIsAvailable(state, LookupGlueClassTag(state, class_id))) {
      visible_class_ids.push_back(class_id);
    }
  }

  lua_getglobal(state, "MAX_CLASSES_PER_RACE");
  if (lua_isnumber(state, -1) != 0) {
    const auto lua_class_limit = static_cast<std::size_t>(std::max(
        0, static_cast<int>(lua_tonumber(state, -1))));
    if (visible_class_ids.size() > lua_class_limit) {
      visible_class_ids.resize(lua_class_limit);
    }
  }
  lua_pop(state, 1);

  const auto xml_class_limit =
      GetGlueIndexedWidgetLimit(state, "CharacterCreateClassButton", visible_class_ids.size());
  if (visible_class_ids.size() > xml_class_limit) {
    visible_class_ids.resize(xml_class_limit);
  }
...
std::string_view LookupGlueRaceClientFileString(lua_State *state, const int race_id) {
  if (const auto *dbc = GetDbcLoader(state); dbc != nullptr) {
    if (const auto *race = dbc->chr_races().LookupEntry(static_cast<std::uint32_t>(race_id));
      ...
      // Match the OpenWow glue contract: return the DBC token unchanged and
      // let the loaded Lua/XML choose how it maps that token to icons and
      // scene assets.  An atlas/VFS probe here turns a valid race into nil and
      // makes GlueParent.lua silently fall back to Orc.
...
std::string_view LookupGlueBackgroundRaceToken(lua_State *state, const int race_id) {
  // The active Lua/XML glue owns scene remapping.  Native code must pass the
  // race's DBC token through unchanged so custom races do not acquire a
  // hidden C++ race-ID mapping.
  return LookupGlueRaceClientFileString(state, race_id);
}
...
std::string_view LookupGlueBackgroundClassToken(lua_State *state, const int class_id) {
  ...
  if (const auto *dbc = GetDbcLoader(state); dbc != nullptr) {
    if (const auto *cls = dbc->chr_classes().LookupEntry(6u);
        cls != nullptr && !cls->client_file_string.empty()) {
      return cls->client_file_string;
    }
  }
  if (const auto *info = FindWotlkClassInfo(6); info != nullptr) {
    return info->tag;
  }
  return LookupGlueClassClientFileString(state, 6);
}
...
std::size_t GetGlueIndexedWidgetLimit(lua_State *state, const char *prefix,
  ...
  if (runtime == nullptr || prefix == nullptr || fallback == 0u) {
    return fallback;
  }

  std::size_t count = 0u;
  for (std::size_t index = 1u; index <= fallback; ++index) {
    if (!runtime->HasWidget(std::string(prefix) + std::to_string(index))) {
      break;
    }
    ++count;
  }
  return count == 0u ? fallback : count;
}

std::vector<GlueCreateClassEntry> GetGlueEnumeratedCreateClassesForRace(
    lua_State *state, const int race_id) {
  const auto discovered_classes = GetAvailableCreateClassesForRace(state, race_id);
  std::vector<GlueCreateClassEntry> classes;
  classes.reserve(discovered_classes.size());
  for (const auto &cls : discovered_classes) {
    if (!cls.has_definition ||
        !LuaClassIconTokenIsAvailable(state, LookupGlueClassTag(state, cls.class_id))) {
      continue;
    }
  ...
  lua_getglobal(state, "MAX_CLASSES_PER_RACE");
  if (lua_isnumber(state, -1) != 0) {
    const auto max_classes = static_cast<std::size_t>(std::max(
        0, static_cast<int>(lua_tonumber(state, -1))));
    if (classes.size() > max_classes) {
      classes.resize(max_classes);
    }
  }
  lua_pop(state, 1);

  const auto xml_class_limit =
      GetGlueIndexedWidgetLimit(state, "CharacterCreateClassButton", classes.size());
  if (classes.size() > xml_class_limit) {
    classes.resize(xml_class_limit);
  }
  return classes;
}
...
int LuaGetAvailableRaces(lua_State *state) {
  // The active Classic CharacterCreate.lua consumes alternating name/token
  // values.  The expansion-enabled flag belongs to a different API contract
  // and must not be inserted into this legacy tuple.
  const auto races = GetGlueAvailableRaces(state);
  const int result_count = openwow::ui::ReserveLuaResultCapacity(
      state, races.size(), 2u, "available-race results");
  ...
  for (const auto &race : races) {
    ...
    const std::string_view race_token =
        LookupGlueRaceClientFileString(state, race.race_id);
    PushLuaStringView(state, race_token.empty() ? race.tag : race_token);
  }
  return result_count;
}
...
int LuaGetAvailableClasses(lua_State *state) {
  ...
  if (const auto *dbc = GetDbcLoader(state); dbc != nullptr && !dbc->chr_classes().empty()) {
    ...
    const int selected_race = game_state != nullptr ? game_state->create_race : 0;
    const std::uint8_t expansion_level =
        openwow::net::ClientServices::Instance().GetExpansionLevel();
    if (game_state != nullptr) {
      game_state->available_create_class_ids = class_ids;
    }
    for (const int class_id : class_ids) {
      const auto *cls = dbc->chr_classes().LookupEntry(static_cast<std::uint32_t>(class_id));
      if (cls == nullptr) {
        lua_pushnil(state);
        lua_pushnil(state);
        lua_pushnumber(state, 0.0);
      ...
      lua_pushnumber(
          state,
          (IsValidRaceClass(state, selected_race, class_id) &&
           expansion_level >= LookupGlueClassRequiredExpansionLevel(state, class_id))
              ? 1.0
              : 0.0);
    }
    return result_count;
  ...
  const auto visible_class_ids = GetGlueEnumeratedClassIds(state);
  const int result_count = openwow::ui::ReserveLuaResultCapacity(
      state, visible_class_ids.size(), 3u, "available-class results");
  auto *game_state = GetGameState(state);
  const int selected_race = game_state != nullptr ? game_state->create_race : 0;
  for (const int class_id : visible_class_ids) {
    ...
    lua_pushnumber(state, IsValidRaceClass(state, selected_race, class_id) ? 1.0 : 0.0);
  }
  if (game_state != nullptr) {
    game_state->available_create_class_ids = visible_class_ids;
  }
  return result_count;
}

int LuaGetClassesForRace(lua_State *state) {
  // CharacterCreateEnumerateClasses() consumes the same alternating
  // name/token contract as CharacterCreateEnumerateRaces().
  const auto *gs = GetGameState(state);
  const int race_id = gs != nullptr ? gs->create_race : 1;
  const int sex_id = GetGlueSelectedSexIndex(state);
  const auto classes = GetGlueEnumeratedCreateClassesForRace(state, race_id);
  if (auto *game_state = GetGameState(state); game_state != nullptr) {
    CacheAvailableCreateClassIds(*game_state, classes);
  }
  const int result_count = openwow::ui::ReserveLuaResultCapacity(
      state, classes.size(), 2u, "race-class results");
...
int LuaGetNameForRace(lua_State *state) {
  ...
  lua_pushlstring(state,
                  race_token.data() != nullptr ? race_token.data() : "",
                  race_token.size());
  return 2;
}
...
int LuaGetFactionForRace(lua_State *state) {
  ...
  if (lua_isnumber(state, 1)) {
    index = LuaNumberToZeroBasedU32Index(lua_tonumber(state, 1));
  } else {
    ...
    const auto selected = std::find_if(
        races.begin(), races.end(), [&](const auto &race) {
          return race.race_id == game_state->create_race;
        });
    if (selected == races.end()) {
      lua_pushnil(state);
      lua_pushnil(state);
      return 2;
    }
    index = static_cast<std::uint32_t>(selected - races.begin());
  }
...
int LuaGetSelectedSex(lua_State *state) {
  ...
  const int mapped =
      (sex >= 0 && sex < static_cast<int>(std::size(kClassicLuaSexMapping)))
          ? kClassicLuaSexMapping[sex]
          : kClassicLuaSexMapping[0];
...
static void SelectCharacterCreationRace(lua_State *state,
  ...
  game_state.create_race = race_id;
  CacheAvailableCreateClassIds(
      game_state, GetAvailableCreateClassesForRace(state, game_state.create_race));
...
int LuaSetSelectedSex(lua_State *state) {
  ...
  const std::int32_t sex_id =
      openwow::ui::TruncateLuaNumberToI32(lua_tonumber(state, 1));
  for (int i = 0; i < static_cast<int>(std::size(kClassicLuaSexMapping)); ++i) {
    if (sex_id == kClassicLuaSexMapping[i]) {
      if (gs->create_sex == i) {
        return 0;
      }
...
int LuaIsRaceClassRestricted(lua_State *state) {
  ...
  const auto *gs = GetGameState(state);
  if (gs != nullptr && race_id < gs->race_class_restriction_masks.size()) {
    const std::uint32_t mask =
        gs->race_class_restriction_masks[static_cast<std::size_t>(race_id)];
    const double result = ((mask >> (class_id & 31u)) & 1u) != 0u ? 1.0 : 0.0;
    lua_pushnumber(state, result);
    return 1;
  }

  // Character-list restriction masks only contain the legacy race slots.  For
  // races supplied by the active DBC, use the same CharBaseInfo authority as
  // CharacterCreate instead of rejecting the race by numeric ID.
  const double result = IsValidRaceClass(state,
                                         openwow::ui::SignedI32FromU32Bits(race_id),
                                         openwow::ui::SignedI32FromU32Bits(class_id))
                            ? 0.0
                            : 1.0;
  lua_pushnumber(state, result);
  return 1;
...
int LuaSetCharCustomizeBackground(lua_State *s) {
  ...
  if (gs != nullptr) {
    if (auto *runtime = GetWidgetRuntime(s);
        runtime != nullptr && gs->background_controller != nullptr) {
      gs->background_controller->SetCharCustomizeBackground(
          *gs, *runtime, raw != nullptr ? raw : "");
    }
  }
  return 0;
...
int LuaPaidChange_GetCurrentClassIndex(lua_State *s) {
  ...
  if (gs != nullptr) {
    if (const auto *character = GetCustomizationSourceCharacter(*gs); character != nullptr) {
      const auto class_ids = GetGlueEnumeratedClassIds(s);
...
int LuaGetSelectBackgroundModel(lua_State *state) {
  ...
  const std::int64_t idx = LuaNumberToZeroBasedI32Index(lua_tonumber(state, 1));
  auto *gs = GetGameState(state);
  const auto push_token_or_nil = [state](const std::string_view token) {
    if (token.empty()) {
      lua_pushnil(state);
    } else {
      PushLuaStringView(state, token);
    }
  };
