...
GlueResolvedFontStringStyle ResolveGlueFontStringStyle(
  ...
  const bool has_direct_button_font =
      widget.has_button_font && !widget.button_font_file.empty() &&
      widget.button_font_height_px > 0;
  if (has_direct_button_font) {
    resolved.font.font_file = widget.button_font_file;
    resolved.font.height_px = widget.button_font_height_px;
    const std::string flags_lower = ToLowerAscii(widget.button_font_flags);
    if (flags_lower.find("thickoutline") != std::string::npos ||
        flags_lower.find("thick") != std::string::npos) {
      resolved.font.outline = "THICKOUTLINE";
    } else if (flags_lower.find("outline") != std::string::npos) {
      resolved.font.outline = "OUTLINE";
    }
    resolved.font.monochrome =
        flags_lower.find("monochrome") != std::string::npos;
    resolved.has_bound_font = true;
  }

  const std::string style_tokens =
      widget.font_style.empty() ? widget.inherits : widget.font_style;
  if (!has_direct_button_font && registry != nullptr) {
