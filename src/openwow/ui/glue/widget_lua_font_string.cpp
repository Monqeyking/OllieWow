...
int LuaWidget_SetFont(lua_State* state) {
  const auto name = WidgetNameFromArg(state, 1);
  if (name.empty()) {
    return 0;
  }
  if (lua_isstring(state, 2) == 0 || lua_isnumber(state, 3) == 0) {
    return luaL_error(
        state, "Usage: %s:SetFont(\"font\", fontHeight [, flags])",
        name.c_str());
  }
  ...
  if (!widget.has_value() ||
      (!openwow::text::EqualsIgnoreCaseAscii(widget->kind, "button") &&
       !openwow::text::EqualsIgnoreCaseAscii(widget->kind, "checkbutton"))) {
    return 0;
  }

  const char* path = lua_tostring(state, 2);
  const int height_px = static_cast<int>(std::lround(lua_tonumber(state, 3)));
  const char* flags = luaL_optstring(state, 4, "");
  runtime->SetButtonFont(name, path != nullptr ? path : "", height_px,
                         flags != nullptr ? flags : "");
  return 0;
}
...
int LuaWidget_GetFont(lua_State* state) {
  ...
  if (auto* runtime = GetWidgetRuntime(state); runtime != nullptr && !name.empty()) {
    const auto widget = runtime->GetWidget(name);
    if (widget.has_value()) {
      if (widget->has_button_font && !widget->button_font_file.empty() &&
          widget->button_font_height_px > 0) {
        lua_pushstring(state, widget->button_font_file.c_str());
        lua_pushnumber(state, widget->button_font_height_px);
        lua_pushstring(state, widget->button_font_flags.c_str());
        return 3;
      }
