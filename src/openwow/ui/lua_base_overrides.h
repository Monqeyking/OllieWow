...
inline std::optional<std::string> RewriteLua50TableIterators(
  ...
  for (std::size_t pos = 0; pos < chunk.size();) {
    ...
    const auto do_pos = FindLuaDoToken(chunk, pos + 3u);
    if (!do_pos.has_value()) {
      ++pos;
      continue;
    }

    std::size_t in_pos = pos + 3u;
    while (in_pos + 2u < *do_pos) {
      if (chunk[in_pos] == '-' && in_pos + 1u < chunk.size() &&
          chunk[in_pos + 1u] == '-') {
        in_pos = SkipLuaComment(chunk, in_pos);
        continue;
      }
      if (chunk[in_pos] == '\'' || chunk[in_pos] == '"') {
        in_pos = SkipLuaQuotedString(chunk, in_pos);
        continue;
      }
      if (LuaTokenAt(chunk, in_pos, "in")) {
        break;
      }
      ++in_pos;
    }
    if (in_pos + 2u >= *do_pos || !LuaTokenAt(chunk, in_pos, "in")) {
      pos = *do_pos + 2u;
      continue;
    }
    ...
    edits.push_back({iterable_begin, iterable_begin, "pairs("});
    edits.push_back({iterable_end, iterable_end, ")"});
    pos = *do_pos + 2u;
  }
  ...
  std::string rewritten;
  rewritten.reserve(chunk.size() + edits.size() * 6u);
  std::size_t cursor = 0;
  for (const auto& edit : edits) {
    if (edit.begin < cursor) {
      continue;
...
inline int LoadClientLuaChunk(lua_State* state, std::string_view chunk, const char* chunk_name) {
  ...
  chunk = openwow::text::StripUtf8Bom(chunk);
  if (const auto lua50_chunk = detail::RewriteLua50TableIterators(chunk);
      lua50_chunk.has_value()) {
    return luaL_loadbuffer(state, lua50_chunk->data(), lua50_chunk->size(),
