...
void GlueClient::HandleMouseUp(const SDL_Event &event) {
  ...
  if (mode_ != UiMode::kInWorld && !pressed_widget_name_.empty()) {
    ...
    // A locked highlight is a separate selection visual.  It must not keep the
    // button in PUSHED state after mouse release: that state hides the
    // NormalTexture and leaves the selected icon as an empty/black square.
    if (!openwow::text::EqualsIgnoreCaseAscii(glue_widgets_.GetButtonState(released),
                                               "DISABLED")) {
