...
void ApplyCharacterListResult(GlueFlowContext& ctx,
  ...
  openwow::diagnostics::Log(
      openwow::diagnostics::LogLevel::kInfo,
      "GlueFlow: character enum received (count=" +
          std::to_string(result.characters.size()) + ")");
  ...
  if (gs.char_select_scene != nullptr) {
    ...
    openwow::diagnostics::Log(
        openwow::diagnostics::LogLevel::kInfo,
        "GlueFlow: selected character body model=" +
            (gs.char_select_scene->selected_character_model_path().empty()
                 ? std::string("<none>")
                 : gs.char_select_scene->selected_character_model_path()));
  }
}
