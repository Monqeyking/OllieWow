  ...
  if (scenario == "glue_character_create_regression") {
    ...
    const auto validate_icon_regions = [&](const char* prefix,
      ...
      for (int index = 1; index <= maximum; ++index) {
        ...
        ++visible_buttons;
        for (const char* suffix : {"NormalTexture", "HighlightTexture"}) {
      ...
      if (visible_buttons == 0) {
        return std::string(prefix) + " has no visible button(s)";
      }
      if (normal_icon_rects.size() != static_cast<std::size_t>(visible_buttons)) {
        return std::string(prefix) + " has only " +
               std::to_string(normal_icon_rects.size()) + " of " +
               std::to_string(visible_buttons) +
               " distinct atlas rectangles for visible buttons";
      }
      return std::nullopt;
    };
    const auto validate_selected_race_icon = [&]() -> std::optional<std::string> {
      const auto icon = ctx->glue_widgets->GetWidget("CharacterCreateRaceIcon");
      if (!icon.has_value()) {
        return "CharacterCreateRaceIcon is missing";
      }
      if (!ctx->glue_widgets->IsVisible("CharacterCreateRaceIcon")) {
        return "CharacterCreateRaceIcon is hidden after race selection";
      }
      if (icon->texture_file.empty()) {
        return "CharacterCreateRaceIcon has no texture after race selection";
      }
      if (!(icon->tex_right > icon->tex_left && icon->tex_bottom > icon->tex_top)) {
        return "CharacterCreateRaceIcon has degenerate texcoords after race selection";
      }
      return std::nullopt;
    };
    ...
    if (step_ == 1 && elapsed >= 1500) {
      ...
      if (const auto icon_error =
              validate_icon_regions("CharacterCreateClassButton", 8);
          icon_error.has_value()) {
      ...
      if (!run_lua(R"LUA(
        assert(select("#", GetCharacterInfo(1)) == 8,
               "GetCharacterInfo valid result must contain exactly 8 values")
        assert(select("#", GetCharacterInfo(99999)) == 8,
               "GetCharacterInfo invalid result must contain exactly 8 values")
        if GetNumCharacters() > 0 then
          local _, _, _, _, _, fileString, gender, ghost = GetCharacterInfo(1)
          assert(type(fileString) == "string" and fileString ~= "",
                 "GetCharacterInfo must return the DBC race client_file_string in position 6")
          assert(gender == 2 or gender == 3,
                 "GetCharacterInfo must return the gender token in position 7")
          assert(type(ghost) == "boolean",
                 "GetCharacterInfo must return ghost state in position 8")
        end
        local raceValues = { GetAvailableRaces() }
        assert(#raceValues % 2 == 0,
               "GetAvailableRaces must return name/token pairs")
        for index = 1, #raceValues, 2 do
          assert(type(raceValues[index]) == "string" and
                 type(raceValues[index + 1]) == "string" and
                 raceValues[index] ~= "" and raceValues[index + 1] ~= "",
                 "GetAvailableRaces returned an invalid pair at " .. index)
        end
        local classValues = { GetAvailableClasses() }
        assert(#classValues % 3 == 0,
               "GetAvailableClasses must return name, token, enabled triples")
        for index = 1, #classValues, 3 do
          assert(type(classValues[index]) == "string" and
                 type(classValues[index + 1]) == "string" and
                 type(classValues[index + 2]) == "number",
                 "GetAvailableClasses returned an invalid triple at " .. index)
        end
      )LUA", "ScenarioCharacterInfoReturnContract.lua")) {
        fail("GetCharacterInfo return contract changed");
        return true;
      }
      ...
      if (ctx->game_state->char_customize_background.empty()) {
        fail("CharacterCreate background was not selected by the Glue Lua flow");
        return true;
      }
    ...
    if (step_ == 3 && elapsed >= 5500) {
      if (!run_lua(R"LUA(
        if CharacterCreateTooltip and CharacterCreateTooltip.Hide then
    ...
    if (step_ == 4 && elapsed >= 9000) {
      ...
      if (const auto icon_error = validate_selected_race_icon();
          icon_error.has_value()) {
        fail("selected race icon: " + *icon_error);
        return true;
      }
      if (!openwow::text::EqualsIgnoreCaseAscii(
              ctx->glue_widgets->GetButtonState("CharacterCreateRaceButton2"),
              "NORMAL")) {
        fail("selected race button remained pushed after release");
        return true;
      }
      if (!ctx->glue_widgets->IsVisible("CharacterCreateRaceButton2NormalTexture")) {
        fail("selected race button normal texture is hidden after release");
        return true;
      }
      request_capture();
      step_ = 5;
    ...
    if (step_ == 5 && elapsed >= 9500) {
      ...
      if (!run_lua(R"LUA(
        local race_results = {GetAvailableRaces()}
        assert(#race_results > 0 and (#race_results % 2) == 0,
               "GetAvailableRaces must return dynamic name/tag pairs")
        for index = 1, #race_results, 2 do
          assert(type(race_results[index]) == "string" and race_results[index] ~= "",
                 "dynamic race has no display name at result " .. index)
          assert(type(race_results[index + 1]) == "string" and race_results[index + 1] ~= "",
                 "dynamic race has no client token at result " .. (index + 1))
        end
        for index = 1, CharacterCreate.numRaces or 0 do
          SetSelectedRace(index)
          SetCharacterRace(index)
          local race_name, race_token = GetNameForRace()
          assert(type(race_name) == "string" and race_name ~= "",
                 "dynamic race has no display name at index " .. index)
          assert(type(race_token) == "string" and race_token ~= "",
                 "dynamic race has no client token at index " .. index)
          local class_results = {GetClassesForRace()}
          assert((#class_results % 2) == 0,
                 "GetClassesForRace must return class name/token pairs")
          for class_index = 1, #class_results, 2 do
            assert(type(class_results[class_index]) == "string" and
                   class_results[class_index] ~= "",
                   "dynamic class has no display name at result " .. class_index)
            assert(type(class_results[class_index + 1]) == "string" and
                   class_results[class_index + 1] ~= "",
                   "dynamic class has no client token at result " .. (class_index + 1))
          end
          CharacterCreateEnumerateClasses(GetClassesForRace())
        end
        local all_class_results = {GetAvailableClasses()}
        assert(#all_class_results > 0 and (#all_class_results % 3) == 0,
               "GetAvailableClasses must return dynamic name/tag/enabled triples")
        for index = 1, #all_class_results, 3 do
          assert(type(all_class_results[index]) == "string" and all_class_results[index] ~= "",
                 "dynamic class has no display name at result " .. index)
          assert(type(all_class_results[index + 1]) == "string" and all_class_results[index + 1] ~= "",
                 "dynamic class has no client token at result " .. (index + 1))
          assert(type(all_class_results[index + 2]) == "number",
                 "dynamic class has no enabled flag at result " .. (index + 2))
        end
        SetSelectedRace(1)
        SetCharacterRace(1)
        ...
        if classIndex and classIndex > 0 then
          ...
          local _, _, selectedClassIndex = GetSelectedClass()
          assert(selectedClassIndex == classIndex,
                 "selected class index did not round-trip through the race class list")
        end
        assert(type(SetCharacterGender) == "function",
               "SetCharacterGender is missing from the Classic glue surface")
        local currentSex = GetSelectedSex()
        local targetSex = currentSex == SEX_MALE and SEX_FEMALE or SEX_MALE
        SetCharacterGender(targetSex)
        assert(GetSelectedSex() == targetSex,
               "gender selection did not publish the requested sex")
        if type(CycleCharCustomization) == "function" then
          CycleCharCustomization(1, 1)
        end
      )LUA", "ScenarioCharacterCreateCycleAll.lua")) {
        fail("cycling all race/class atlas slots or gender/customization failed");
        return true;
      }
      step_ = 6;
      return true;
